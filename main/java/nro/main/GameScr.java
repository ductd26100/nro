...
public class GameScr {
     public static void SendFile(@NotNull Session session, int cmd, String url) throws IOException {
    ...
    public static void reciveImage(@NotNull Player p, @NotNull Message m) throws IOException {
    ...
    public static void reciveImageSmall(@NotNull Player p, @NotNull Message m) throws IOException {
        int id = m.reader().readInt();
        m.cleanup();
        byte[] icon = FileIO.readFile("res/icon/x" + p.session.zoomLevel + "/" + id + ".png");
        assert icon != null;
    ...
    public static @Nullable ByteArrayOutputStream loadFile(String url) {
    ...
    public static void UIshop(@NotNull Player p, TabItemShop[] tabs) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < tabs.length; i++) {
                ...
                for (int j = 0; j < tabItemShop.itemsSell.size(); j++) {
                    ...
                    if((itemSell.item.template.id >= 213 && itemSell.item.template.id <= 219) || itemSell.item.template.id == 522 || itemSell.item.template.id == 671 || itemSell.item.template.id == 672
                            || itemSell.item.template.id == 1144 || itemSell.item.template.id == 1143 || itemSell.item.template.id == 1142) { //Bua tri tue
                        byte _idOption = 65;
                        Amulet _amulet = null;
                        int timeLeft = 0;
                        ...
                        } else if(itemSell.item.template.id == 1144) {
                            _amulet = p.listAmulet.get(10);
                        } else if(itemSell.item.template.id == 1143) {
                            _amulet = p.listAmulet.get(11);
                        } else if(itemSell.item.template.id == 1142) {
                            _amulet = p.listAmulet.get(12);
                        }
