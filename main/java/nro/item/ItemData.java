...
//            FileIO.writeFile("res/cache/NRdataOverWrite", arr);
    ...
    public static void buildNrPart() {
        Util.log("START BUILD CACHE NR_PART");
        PreparedStatement ps;
        ResultSet rs;
        Connection conn = null;
        try {
            conn = DataSource.getConnection();
            ps = conn.prepareStatement("SELECT * FROM parts2");
            conn.setAutoCommit(false);
            rs = ps.executeQuery();
            int id = 0;
            byte type = 0;
            ...
            JSONObject job;
            Part part;
            //TODO INIT ARRAYLIST BYTE PART
            ArrayList<Byte> newPart = new ArrayList<>();
            newPart = addShort(newPart, (short)1197);
            while(rs.next()) {

                id = rs.getInt("id");
                type = rs.getByte("type");
                jar = (JSONArray) JSONValue.parse(rs.getString("pi"));

                newPart.add(type);
                for (Object o : jar) {
                    job = null;
                    job = (JSONObject) JSONValue.parse(o.toString());
                    short idImage = Short.parseShort(job.get("id").toString());
                    byte dx = Byte.parseByte(job.get("dx").toString());
                    byte dy = Byte.parseByte(job.get("dy").toString());
                    newPart = addShort(newPart, idImage);
                    newPart.add(dx);
                    newPart.add(dy);
                    Util.log("PART " +  id + ": ID: " + idImage + ", dx: " + dx + ", dy: " + dy);
                }
            }
            conn.close();

            //TODO CHUYEN ARRAYLIST BYTE SANG ARRAY BYTE
            byte[] arr = new byte[newPart.size()];
            for(int i = 0; i < newPart.size(); i++) {
                arr[i] = newPart.get(i);
            }
//            FileIO.writeFile("res/cache/newcache/NR_part", arr);
            FileIO.writeFile("res/cache/data/v225/NR_part_new", arr);
            Util.log("DONE BUILD NR_PART");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            try {
    ...
    public static void buildCacheCountImage() {
//        try {
//            ByteArrayInputStream is = new ByteArrayInputStream(Objects.requireNonNull(FileIO.readFile("data/msg/v225_-77_r")));
//            DataInputStream dis = new DataInputStream(is);
//            short number = dis.readShort();
//            Util.log("number Image: " + number);
//            Connection conn = DataSource.getConnection();
//            for (short i = 0; i < number; i++) {
//                byte ver = dis.readByte();
//                Util.log("ID: " + i + ", version: " + ver);
//                ItemDAO.insertImageVersionDB(i, ver, conn);
//            }
//        } catch (Exception e) {
//
//        }

        Util.log("START BUILD CACHE IMAGE -77");
        PreparedStatement ps;
        ResultSet rs;
        Connection conn = null;
        try {
            conn = DataSource.getConnection();
            ps = conn.prepareStatement("SELECT * FROM image_version");
            conn.setAutoCommit(false);
            rs = ps.executeQuery();
            byte version = 0;
            //TODO INIT ARRAYLIST BYTE PART
            ArrayList<Byte> newPart = new ArrayList<>();
            newPart = addShort(newPart, (short)11017);
            while(rs.next()) {
            ...
            FileIO.writeFile("res/cache/newcache/vNew_-77_r", arr);
            Util.log("DONE BUILD CACHE IMAGE -77");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            try {
    ...
    public static void insertVersionImage() {
        PreparedStatement ps;
        ResultSet rs;
        Connection conn = null;
        ...
        try {
            ...
            ps = conn.prepareStatement("SELECT * FROM image_version WHERE idimage >= 2956 AND idimage <= 2986");
            conn.setAutoCommit(false);
            rs = ps.executeQuery();
            byte version;
            int id;
            ...
            while(rs.next()) {
                ...
                ps = conn2.prepareStatement("INSERT INTO image_version (idimage, version) VALUES (?,?)", Statement.RETURN_GENERATED_KEYS);
                conn2.setAutoCommit(false);
                ps.setInt(1, idInsert);
                ps.setByte(2, version);
                ps.executeUpdate();
                conn2.commit();
                idInsert++;
            }
    ...
    public static void buildNewNRdata() {
        try {
            ByteArrayInputStream is = new ByteArrayInputStream(Objects.requireNonNull(FileIO.readFile("res/cache/data/v225/NR_dart")));
            DataInputStream dis = new DataInputStream(is);
            ArrayList<Byte> bytes = new ArrayList<>();
            bytes.add((byte)3);
            byte[] cache = FileIO.readFile("res/cache/data/v225/NR_dart");
            bytes = addInt(bytes, cache.length);
            while(dis.available() > 0) {
                bytes.add(dis.readByte());
            }
            is = new ByteArrayInputStream(Objects.requireNonNull(FileIO.readFile("res/cache/data/v225/NR_arrow")));
            dis = new DataInputStream(is);
            byte[] cache2 = FileIO.readFile("res/cache/data/v225/NR_arrow");
            bytes = addInt(bytes, cache2.length);
            while(dis.available() > 0) {
                bytes.add(dis.readByte());
            }

            is = new ByteArrayInputStream(Objects.requireNonNull(FileIO.readFile("res/cache/data/v225/NR_effect")));
            dis = new DataInputStream(is);
            byte[] cache3 = FileIO.readFile("res/cache/data/v225/NR_effect");
            bytes = addInt(bytes, cache3.length);
            while(dis.available() > 0) {
                bytes.add(dis.readByte());
            }

            is = new ByteArrayInputStream(Objects.requireNonNull(FileIO.readFile("res/cache/data/v225/NR_image")));
            dis = new DataInputStream(is);
            byte[] cache4 = FileIO.readFile("res/cache/data/v225/NR_image");
            bytes = addInt(bytes, cache4.length);
            while(dis.available() > 0) {
                bytes.add(dis.readByte());
            }

            is = new ByteArrayInputStream(Objects.requireNonNull(FileIO.readFile("res/cache/data/v225/NR_part_new")));
            dis = new DataInputStream(is);
            byte[] cache5 = FileIO.readFile("res/cache/data/v225/NR_part_new");
            bytes = addInt(bytes, cache5.length);
            while(dis.available() > 0) {
                bytes.add(dis.readByte());
            }

            is = new ByteArrayInputStream(Objects.requireNonNull(FileIO.readFile("res/cache/data/v225/NR_skill")));
            dis = new DataInputStream(is);
            byte[] cache6 = FileIO.readFile("res/cache/data/v225/NR_skill");
            bytes = addInt(bytes, cache6.length);
            while(dis.available() > 0) {
                bytes.add(dis.readByte());
            }
            ...
            FileIO.writeFile("res/cache/newcache/NR_data", arr);
            Util.log("DONE!!!");
        } catch (Exception e) {
    ...
    //TODO ADD SHORT TO ARRAYLIST BYTE
    public static ArrayList<Byte> addShort(ArrayList<Byte> arrList, short number) {
    ...
    //TODO ADD INT TO ARRAYLIST BYTE
    public static ArrayList<Byte> addInt(ArrayList<Byte> arrList, int number) {
        ByteBuffer b = ByteBuffer.allocate(4);
        b.putInt(number);
        byte[] result = b.array();
        for (byte value : result) {
