...
public class Item {
    ...
    public boolean isItemBoss = false;
    public boolean itemPay = false;
    ...
    public Item(@NotNull Item _item) { //INIT ITEM TU TEMPLATE
        this.id = _item.id;
...
//        this.itemOptions = _item.itemOptions;
    ...
    public String getInfo() {
        ...
        for (ItemOption itemOption : itemOptions) {
            strInfo.append(itemOption.getOptionString());
        }
        return strInfo.toString();
    }

    public String getInfoUpStar(Player _player) {
        StringBuilder strInfo = new StringBuilder(this.template.name);
        ...
        for (ItemOption itemOption : itemOptions) {
            if(itemOption.id == 107 && itemOption.param < 8) {
                ...
                strInfo.append("\b|0|").append(itemOption.param + 1).append(" Sao Pha Lê");
                hasStar = true;
            } else if(itemOption.id != 102) {
                strInfo.append("\b|0|").append(itemOption.getOptionString());
            }
        }
        if(!hasStar) {
            strInfo.append("\b|0|1 Sao Pha Lê");
        }
        if(starItem == 7) {
            strInfo.append("\b|2|Tỉ lệ thành công 0.5%");
        } else {
            strInfo.append("\b|2|Tỉ lệ thành công ").append(getPercentUpStar(starItem)).append("%");
        }
        if(_player.vang >= (getGoldUpStar(starItem)*1000000L)) {
            strInfo.append("\b|2|Cần ").append(getGoldUpStar(starItem)).append(" Tr vàng");
        } else {
            strInfo.append("\b|7|Cần ").append(getGoldUpStar(starItem)).append(" Tr vàng");
        }
        return strInfo.toString();
    }
    ...
    public String getInfoEpStar(Player _player, int _idEp) {
        ...
        for (ItemOption itemOption : itemOptions) {
            if(itemOption.id == _idOption && itemOption.param < 8*_PerStar) {
                ...
                strInfo.append("\b|0|").append(itOpNew.getOptionString());
                hasStar = true;
        ...
        if(!hasStar) {
            ...
            strInfo.append("\b|0|").append(itOpNew2.getOptionString());
        }
        if(_player.ngoc >= 10) {
            strInfo.append("\b|2|Cần 10 ngọc");
        } else {
            strInfo.append("\b|7|Cần 10 ngọc");
        }
        return strInfo.toString();
    }

    public String getInfoUpgradeItem(Player _player, int _idEp, byte _indexStone) {
        StringBuilder strInfo = new StringBuilder(this.template.name + "\b|2|Hiện tại");
        int levelUp = 0;
        for (ItemOption itemOption : itemOptions) {
        ...
        strInfo.append("\b|2|Sau khi nâng cấp (+").append(levelUp + 1).append(")");
        if(getParamItemByID(mapStoneUpToOption(_idEp)) != 0) {
            for (ItemOption itemOption : itemOptions) {
                if(itemOption.id == mapStoneUpToOption(_idEp)) {
                    strInfo.append("\b|1|").append(mapPointTypeUpByIdStone(_idEp)).append("+").append((int) Math.ceil(itemOption.param * 1.1));
                } else if(itemOption.id != 102 && itemOption.id != 107) {
                    strInfo.append("\b|1|").append(itemOption.getOptionString());
                }
            }
        } else if(_idEp == 222) {
            for (ItemOption itemOption : itemOptions) {
                if(itemOption.id == 22) {
                    strInfo.append("\b|1|").append(mapPointTypeUpByIdStone(_idEp)).append("+").append((int) Math.ceil(itemOption.param * 1.1) * 1000);
                } else if(itemOption.id != 102 && itemOption.id != 107) {
                    strInfo.append("\b|1|").append(itemOption.getOptionString());
                }
            }
        ...
        strInfo.append("\b|2|Tỉ lệ thành công: ").append(getPercentUpItem(levelUp)).append("%");
        if(_player.ItemBag[_indexStone].quantity >= getCountStoneUpItem(levelUp)) {
            strInfo.append("\b|2|Cần ").append(getCountStoneUpItem(levelUp)).append(" Đá ").append(mapTypeStone(_idEp));
        } else {
            strInfo.append("\b|7|Cần ").append(getCountStoneUpItem(levelUp)).append(" Đá ").append(mapTypeStone(_idEp));
        }
        if(_player.vang >= getGoldUpItem(levelUp)) {
            strInfo.append("\b|2|Cần ").append(getGoldUpItem(levelUp)).append("k vàng");
        } else {
            strInfo.append("\b|7|Cần ").append(getGoldUpItem(levelUp)).append("k vàng");
        }
        return strInfo.toString();
    }
    ...
    //TODO CHECK ITEM CO PHAI DO THAN LINH KHONG
    public boolean isItemGod() {
        return this.template.id >= 555 && this.template.id <= 567;
    }

    //TODO: CHECK ITEM CO PHAI TRANG BI KICH HOAT HAY KHONG VA CO THE NANG CAP HAY KHONG
    public boolean isItemActive() {
        for (ItemOption o: this.itemOptions) {
            if (o.id == 128) {
                for (ItemOption o2: this.itemOptions) {
                    if(o2.id == 140) {
                        return true;
                    }
                }
    ...
    //TODO: LAY TEXT THONG TIN NANG CAP TRANG BI KICH HOAT
    public String getInfoUpItemActive(Player p) {
        StringBuilder strInfo = new StringBuilder(this.template.name + "\b|2|Hiện tại");
        for (ItemOption itemOption : itemOptions) {
            if(itemOption.id != 102 && itemOption.id != 107) {
                strInfo.append("\b|0|").append(itemOption.getOptionString());
        ...
        strInfo.append("\b|2|Sau khi nâng cấp");
        int bonusActive = getParamItemByID(215);
        for (ItemOption itemOption : itemOptions) {
            if(itemOption.id != 102 && itemOption.id != 107 && itemOption.id != 215) {
                strInfo.append("\b|1|").append(itemOption.getOptionString());
            }
        }

        if(bonusActive != 0) {
            strInfo.append("\b|1|+").append(bonusActive + 1).append("% Tác dụng set kích hoạt");
        } else {
            strInfo.append("\b|1|+1% Tác dụng set kích hoạt");
        }

        strInfo.append("\b|2|Tỉ lệ thành công: ").append(getPercentUpActive(bonusActive)).append("%");

        if(p.vang >= Constant.MAX_MONEY) {
            strInfo.append("\b|2|Cần 2 Tỷ vàng");
        } else {
            strInfo.append("\b|7|Cần 2 Tỷ vàng");
        }
        return strInfo.toString();
    }

    //TODO: LAY TI LE NANG CAP TRANG BI KICH HOAT
    public int getPercentUpActive(int _levelNow) {
        if (_levelNow == 0) {
            return 20;
        }  else if(_levelNow == 1) {
            return 18;
