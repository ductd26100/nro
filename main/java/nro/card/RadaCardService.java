...
public class RadaCardService {
    ...
    public void setUseCard(@NotNull Player p, short id) {
    ...
    public void setCardLogout(@NotNull Player p, short id) {
        RadaCard card = p.cardByIdCard(id);
        if(card.level >= (byte)1) {
            card.set_use = (byte)0;
            p.cCardUse = (byte)(p.cCardUse - 1) < (byte)0 ? (byte)0 : (byte)(p.cCardUse - 1);
    ...
    public void updateBuffRada(Player p, @NotNull RadaCard card) {
        if(card.set_use == (byte)0) {
    ...
    public void setAmountCard(@NotNull Player p, short id) {
        RadaCard card = p.cardByIdCard(id);
        if(card != null) {
            card.amount++;
