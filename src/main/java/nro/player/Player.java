...
public class Player {
    ...
    public int goldStone = 0;
    public int goldTransaction = 0;
    public int giftEvent = 0;
...
&& this.ItemBody[5].template.id != 639 && this.ItemBody[5].template.id != 640 && this.ItemBody[5].template.id != 641)) {
    ...
    public int getHpFull() {
        ...
        if(this.getSetKichHoatFull() == (byte)9) {
            hp = (int)(hp*Util.getPercentDouble(180 + this.getDameUpItemActive()));
        }
    ...
    public int getMpFull() {
        ...
        //CHECK SET KICH HOAT KI NAMEC
        if(this.getSetKichHoatFull() == (byte)4) {
            mp = (int)(mp*Util.getPercentDouble(200 + this.getDameUpItemActive()));
        }
        if(this.timeEndNRSD[(byte)1] > System.currentTimeMillis()) {
            mp = (int)(mp*1.2);
        }
...
//        }
    ...
    public void hasInjured(int damage) {
        this.hp -= damage;
        if(this.hp <= 0) {
            this.isdie = true;
            this.isTTNL = false;
            this.hp = 0;
            if(this.xHPSaoDen >= (byte)0) {
                this.xHPSaoDen = (byte)0;
            }
        }
    }
        ...
        for(i = 0; i < this.ItemBag.length; ++i) {
            ...
            if(item != null && item.id == id) {
                if((item.id == 590 || (item.id >= 1066 && item.id <= 1070)) && item.quantity < 9999) {
                    return i;
                } else if(item.id == 457 && item.quantity < 99999) {
                    return i;
...
//        System.out.println("QUANTITY ITEM BUY : " + item.quantityTemp);
        try {
            if(item.template.id == 457 && !item.itemPay) {
                return false;
            }
            ...
            if(item.template.id == 521) { //TDLT
                if(index != -1) {
                    for(int i = 0; i < ItemBag[index].itemOptions.size(); i++) {
                        if(ItemBag[index].itemOptions.get(i).id == 1) {
                            ItemBag[index].itemOptions.get(i).param = Math.min((ItemBag[index].itemOptions.get(i).param + 20), 2000);
                        }
                    }
                } else {
                    ...
                    if(countLeft > 0) {
                        ...
                        if (getBagNull() == 0) {
                            ...
                            if(this.ItemBag[index].quantity < 9999) {
                                this.ItemBag[index].quantity = Math.min((this.ItemBag[index].quantity + item.quantity), 9999);
                            }
                            Item _item = new Item(item);
                    ...
                    for(int i = 0; i < ItemBag[index].itemOptions.size(); i++) {
                        if(ItemBag[index].itemOptions.get(i).id == 31) {
                            if(item.isDrop == (byte)1) {
                                ItemBag[index].itemOptions.get(i).param = Math.min((ItemBag[index].itemOptions.get(i).param + 1), 9999);
                            } else {
                                ItemBag[index].itemOptions.get(i).param = Math.min((ItemBag[index].itemOptions.get(i).param + 10), 9999);
                            }
                        }
                    }
                } else {
                ...
                if((indexThucAn != -1 && this.ItemBag[indexThucAn].quantity >= 99) || item.isItemBoss) {
                    if (getBagNull() == 0) {
                        this.sendAddchatYellow("Hành trang không đủ chỗ trống!");
                        return false;
                    } else {
...
//            }
            ...
            || item.template.type == 15 || item.template.type == 16 || item.template.type == 8 || item.template.type == 33 || item.template.type == 25)) { //12 nr
                int sumQuantity = item.template.id == 457 ? 99999 : 99;
                int countLeft = this.ItemBag[index].quantity + item.quantity - sumQuantity;
                if(countLeft > 0) {
                    byte index2 = this.getIndexBagNotItem();
                    if (getBagNull() == 0) {
                        this.sendAddchatYellow("Hành trang không đủ chỗ trống!");
                        ...
                        if(this.ItemBag[index].quantity < sumQuantity) {
                            this.ItemBag[index].quantity = Math.min((this.ItemBag[index].quantity + item.quantity), sumQuantity);
                        }
                        Item _item = new Item(item);
                        _item.quantity += (countLeft - 1);
                        this.ItemBag[index2] = _item;
    ...
    public void itemBagToBox(int index){
        ...
        if(item != null){
            ...
            if(item.template.id == 590 || (item.template.id >= 1066 && item.template.id <= 1070)) { //BI KIEP
                    ...
                    if(countLeft > 0) {
                        ...
                        if (getBoxNull() == 0) {
                            ...
                            if(this.ItemBox[indexBox].quantity < 9999) {
                                this.ItemBox[indexBox].quantity = Math.min((this.ItemBox[indexBox].quantity + item.quantity), 9999);
                            }
                            Item _item = new Item(item);
                            _item.quantity += (countLeft - 1);
                ...
                if(indexBox != -1) {
                    ...
                    for(int i = 0; i < ItemBox[indexBox].itemOptions.size(); i++) {
                        if(ItemBox[indexBox].itemOptions.get(i).id == 31) {
                            ItemBox[indexBox].itemOptions.get(i).param = Math.min((ItemBox[indexBox].itemOptions.get(i).param + item.getParamItemByID(31)), 9999);
                        }
                    }
                } else {
                    indexBox = this.getIndexBoxNotItem();
                ...
                int sumQuantity = item.template.id == 457 ? 99999 : 99;
                int countLeft = ItemBox[indexBox].quantity + item.quantity - sumQuantity;
                ...
                if(countLeft > 0) {
                    ...
                    if (getBoxNull() <= 0) {
                        ...
                        if(ItemBox[indexBox].quantity < sumQuantity) {
                            ItemBox[indexBox].quantity = Math.min((ItemBox[indexBox].quantity + item.quantity), sumQuantity);
                        }
                        Item _item = new Item(item);
                        _item.quantity += (countLeft - 1);
                        this.ItemBox[indexBox2] = _item;
...
//        }
        ...
        if(item != null){
            ...
            if (indexBag != -1 && (item.template.type == 27 || item.template.type == 6 || item.template.type == 12 || item.template.type == 14 || item.template.type == 15
                ...
                item.template.type == 29 || item.template.type == 30 || item.template.type == 31 || item.template.type == 33)) {
                int sumQuantity = item.template.id == 457 ? 99999 : 99;
                int countLeft = ItemBag[indexBag].quantity + item.quantity - sumQuantity;
                if(countLeft > 0) {
                    ...
                    if (getBagNull() <= 0) {
                        ...
                        if(ItemBag[indexBag].quantity < sumQuantity) {
                            ItemBag[indexBag].quantity = Math.min((ItemBag[indexBag].quantity + item.quantity), sumQuantity);
                        }
                        Item _item = new Item(item);
                        _item.quantity += (countLeft - 1);
            ...
            if (_item.template.type == (byte)32) {
                ...
                if(cMinute > 0) {
                    if(_item.id == 531 || _item.id ==536) {
                        bonusGLT = (byte)3;
                    } else if(_item.id == 530 || _item.id ==535) {
                        bonusGLT = (byte)2;
                    } else {
                        bonusGLT = (byte)1;
                    }
...
//            System.out.println("ismonkey body to bag: " + this.isMonkey);
    ...
    public void itemBagToBody(byte index) throws IOException {
        Item item = this.ItemBag[(byte)index];
        if(item != null) {
...
//                                System.out.println("pet het khi");
                                detu.isMonkey = false;
                                detu.hp = Math.min(detu.hp, detu.getHpFull());
...
//                                System.out.println("pet het khi");
   ...
   public void removeItemBag(int index){
    ...
    public void loadBox() throws IOException {
        ...
        for (Item itemBox : this.ItemBox) {
            m.writer().writeShort(itemBox.id);
            m.writer().writeInt(itemBox.quantity);
            m.writer().writeUTF(itemBox.getInfo());
            m.writer().writeUTF(itemBox.getContent());
            m.writer().writeByte(itemBox.itemOptions.size());
            for (int j = 0; j < itemBox.itemOptions.size(); j++) {
                m.writer().writeByte(0);
    ...
    public void sortBag() throws IOException {
        try {
            ...
            for (i = 0; i < ItemBag.length; i = (i + 1)) {
                if (ItemBag[i] != null && !(ItemBag[i]).isExpires) {
                    assert ItemTemplate.ItemTemplateID(ItemBag[i].id) != null;
                    if ((ItemTemplate.ItemTemplateID(ItemBag[i].id)).isUpToUp) {
    ...
    public void sortBox() throws IOException {
        for (byte i = 0; i < this.ItemBox.length; ++i) {
            if (this.ItemBox[i] != null && !this.ItemBox[i].isExpires) {
                assert ItemTemplate.ItemTemplateID(this.ItemBox[i].id) != null;
                if (ItemTemplate.ItemTemplateID(this.ItemBox[i].id).isUpToUp) {
...
//        Skill skl = getSkillById(13);
    ...
    //                                System.out.println("pet het khi");
                                    ...
                                    _detu.hp = Math.min(_detu.hp, _detu.getHpFull());
                                    updateHpDetu(_player, _detu);
                                    //send default part to all player
                                    for (Player p : _player.zone.players) {
        ...
        if(!this.isdie) {
            ...
            if(step == 1) {
                ...
                if(this.countCharge < 10 && (this.hp < this.getHpFull() || this.mp < this.getMpFull())) {
                    ...
                    this.hp = Math.min(this.hp, this.getHpFull());
                    this.mp = Math.min(this.mp, this.getMpFull());
...
//                    this.zone.setSkillPaint(skillID, this, 6);
    ...
    //tai tao nang luong
    public void petChargeHPMP(Player _player, @NotNull Detu _detu, int step) {
        if(!_detu.isdie) {
            ...
            if(step == 1) {
                ...
                if(_detu.countCharge < 10 && (_detu.hp < _detu.getHpFull() || _detu.mp < _detu.getMpFull())) {
                    ...
                    _detu.hp = Math.min(_detu.hp, _detu.getHpFull());
                    _detu.mp = Math.min(_detu.mp, _detu.getMpFull());
...
//                    _player.zone.setSkillPaint(skillID, _detu, 6);
    ...
    public void bossChargeHPMP(@NotNull Boss _boss, int step) {
        if(!_boss.isdie) {
            SkillData skilldata = new SkillData();
...
//                if(_boss.countCharge < 10 && (_boss.hp < _boss.hpFull)) {
                if((_boss.hp < _boss.hpFull)) {
                    ...
                    _boss.hp = Math.min(_boss.hp, _boss.hpFull);
...
//            _boss.sendAddchatYellow("Không thể thực hiện");
    ...
    public void bossNoPetChargeHPMP(@NotNull Boss _boss, int step) {
        if(!_boss.isdie) {
            SkillData skilldata = new SkillData();
            short skillID = skilldata.getSkillIDBySkillTemplateID((byte)2, (short)8, 7);
            if(step == 1) {
...
//            }
    ...
    public int getPercentHutHp() {
        ...
        if(ItemBody[5] != null) {
            _percent += ItemBody[5].getParamItemByID(95); //spl hut mau
        }
        if(ItemBody[6] != null){
    ...
    public int getPercentHutKi() {
        ...
        if(ItemBody[5] != null){
            _percent += ItemBody[5].getParamItemByID(96); //spl hut mau
        }
        if(ItemBody[6] != null){
    ...
    public int getPercentPST() {
        ...
        if(ItemBody[5] != null){
            _percent += ItemBody[5].getParamItemByID(97); //spl pst
        }
        if(ItemBody[6] != null){
    ...
    public int getPercentGold() {
        ...
        if(ItemBody[5] != null){
            _percent += ItemBody[5].getParamItemByID(100); //spl pst
        }
        if(ItemBody[6] != null){
    ...
    public int dameUpByCaiTrang(int damage) {
        if(ItemBody[8] != null && ItemBody[8].template.id == 1001) {
            damage += (int)(damage*Util.getPercentDouble(ItemBody[8].getParamItemByID(5)));
        }
        return damage;
    }

    public int dameUpByItemLung(int damage) {
    ...
    public boolean getBuaDetu() {
        return listAmulet.get(7).timeEnd > System.currentTimeMillis();
    }
    public boolean getBuaDetux3() {
        return listAmulet.get(10).timeEnd > System.currentTimeMillis();
    }
    public boolean getBuaTriTueMabu() {
        return listAmulet.get(11).timeEnd > System.currentTimeMillis();
    }
    public boolean getBuaSucManhMabu() {
        return listAmulet.get(12).timeEnd > System.currentTimeMillis();
    }
    public boolean getBuaManhMe() {
        return listAmulet.get(1).timeEnd > System.currentTimeMillis();
    }
    public boolean getBuaDaTrau() {
        return listAmulet.get(2).timeEnd > System.currentTimeMillis();
    }
    public boolean getBuaOaiHung() {
        return listAmulet.get(3).timeEnd > System.currentTimeMillis();
    }
    public boolean getBuaBatTu() {
        return listAmulet.get(4).timeEnd > System.currentTimeMillis();
    }
    public boolean getBuaDeoDai() {
        return listAmulet.get(5).timeEnd > System.currentTimeMillis();
    }
    public boolean getBuaThuHut() {
        return listAmulet.get(6).timeEnd > System.currentTimeMillis();
    }
    // END GET BUA
            ...
            for(byte i = 0; i < this.ItemBody[0].itemOptions.size(); i++) {
                if(this.ItemBody[0].itemOptions.get(i).id == 127 || this.ItemBody[0].itemOptions.get(i).id == 128 || this.ItemBody[0].itemOptions.get(i).id == 129
                   ...
                   || this.ItemBody[0].itemOptions.get(i).id == 133 || this.ItemBody[0].itemOptions.get(i).id == 134 || this.ItemBody[0].itemOptions.get(i).id == 135
                        || this.ItemBody[0].itemOptions.get(i).id == 213) {
    ...
    public boolean chkRadaSetKichHoat(@NotNull Item item) {
    ...
    public static @Nullable Player setup(int account_id) {
        Connection conn = null;
        try {
            conn = DataSource.getConnection();
//            synchronized (Server.LOCK_MYSQL) {
                ResultSet rs = conn.prepareStatement("SELECT * FROM `player` LEFT JOIN clan_member ON player.account_id = clan_member.player_id WHERE player.account_id=" + account_id + ";").executeQuery();
//                ResultSet rs = SQLManager.stat.executeQuery("SELECT * FROM `player` WHERE `account_id`LIKE'" + account_id + "';");
//                ResultSet rs = SQLManager.stat.executeQuery("SELECT * FROM `player` LEFT JOIN clan_member ON player.account_id = clan_member.player_id WHERE player.account_id=" + account_id + ";");
                if (rs != null && rs.first()) {
                    ...
                    if(player.name.equals("")) {
                        player.name = "toidaidot" + player.id;
                    }
...
//                    Map map = Manager.getMapid(rs.getByte("where_id"));
                    ...
                    player.map = MainManager.getMapid(rs.getInt("where_id"));
...
//                    player.rolePT = rs.getByte("role_pt");
                    if(rs.getObject("role") != null) {
                        player.rolePT = rs.getByte("role");
                    } else {
                        player.rolePT = (byte)2;
                    }
                    player.isPorata2 = rs.getByte("nhapthe2") == 1;
                    player.hasTrungMabu = rs.getByte("hasmabu") == 1;
                    player.isTennis = rs.getByte("istennis") == 1;
...
//                        for (j = 0; j < player.maxluggage; j++) {
                    ...
                    if (jar != null) {
                        for (j = 0; j < jar.size(); ++j) {
                            ...
                            if(player.levelTree > (byte) 10) {
                                player.levelTree = (byte)10;
                            }
                            player.lastTimeTree = Long.parseLong(job2.get("time").toString());
                            player.maxBean = Byte.parseByte(job2.get("max").toString());
                            if(player.lastTimeTree > (byte) 23) {
                                player.lastTimeTree = (byte)23;
                            }
                    ...
                    if (jar != null) {
                        for (j = 0; j < jar.size(); ++j) {
                            ...
                            boolean isOn = PlayerManger.gI().getPlayerByUserID(idFriend) != null;
...
//                    rs = SQLManager.stat.executeQuery("SELECT * FROM `pet` WHERE `account_id`=" + account_id + ";");
                    rs = conn.prepareStatement("SELECT * FROM `pet` WHERE `account_id`=" + account_id + ";").executeQuery();
                    if (rs != null && rs.first()) {
                        player.havePet = 1;
...
//            }
    ...
    public static @Nullable Player setup2(ResultSet rs) {
        Connection conn = null;
        try {
//            synchronized (Server.LOCK_MYSQL) {
//            ResultSet rs = conn.prepareStatement("SELECT * FROM `player` LEFT JOIN clan_member ON player.account_id = clan_member.player_id WHERE player.account_id=" + account_id + ";").executeQuery();
...
//                        for (j = 0; j < player.maxluggage; j++) {
                ...
                player.countMoNoiTai = (byte)(rs.getInt("count_noitai"));
                conn = DataSource.getConnection();
                rs = conn.prepareStatement("SELECT * FROM `pet` WHERE `account_id`=" + player.id + ";").executeQuery();
                if (rs != null && rs.first()) {
                    player.havePet = 1;
                    player.detu = new Detu();
//                        player.detu.id = (-player.id - 100000);
...
//            }
    ...
    public boolean checkCTCold() {
        return ItemBody[5] != null && (ItemBody[5].template.id == 450 || ItemBody[5].template.id == 630 || ItemBody[5].template.id == 631 || ItemBody[5].template.id == 632
    ...
    public boolean checkFullSetThan() {
        if(this.ItemBody[0] != null && this.ItemBody[1] != null && this.ItemBody[2] != null && this.ItemBody[3] != null && this.ItemBody[4] != null) {
            return this.ItemBody[0].template.level == (byte) 13 && this.ItemBody[1].template.level == (byte) 13 && this.ItemBody[2].template.level == (byte) 13
    ...
    public boolean checkSetByLevel(byte level) {
        if(this.ItemBody[0] != null && this.ItemBody[1] != null && this.ItemBody[2] != null && this.ItemBody[3] != null && this.ItemBody[4] != null) {
            return this.ItemBody[0].template.level == level && this.ItemBody[1].template.level == level && this.ItemBody[2].template.level == level
    ...
    //GET OBJ RADA CARD
    public JSONObject ObjectCard(@NotNull RadaCard card) {
        JSONObject put = new JSONObject();
...
//        put.put((Object)"unlock", (Object)card.unlock);
    ...
    public JSONObject ObjectFriend(@NotNull Friend friend) {
        JSONObject put = new JSONObject();
    ...
    //TODO LAY % BONUS SET KICH HOAT
    public int getDameUpItemActive() {
        if(this.ItemBody[0] != null && this.ItemBody[1] != null && this.ItemBody[2] != null && this.ItemBody[3] != null && this.ItemBody[4] != null) {
            int min = this.ItemBody[0].getParamItemByID(215);

            for(byte i = 1; i < 5; i++) {
                min = Math.min(min, this.ItemBody[i].getParamItemByID(215));
            }
            return min;
        }
        return 0;
