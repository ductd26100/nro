...
public class ItemDAO {
    ...
    public static void insertPartDB(int i, byte type, Part part) {
        String INSERT_LOG = "INSERT INTO parts2 (id, type, pi) VALUES (?,?,?)";
        ...
        try {
            ...
            for (int j = 0; j < part.pi.length; j++) {
                ...
                objPart.put("dx", part.pi[j].dx);
                objPart.put("dy", part.pi[j].dy);
                objPart.put("id", part.pi[j].id);
                jarr.add(objPart);
            }
    ...
    public static void insertImageVersionDB(int id, byte ver, Connection conn) {
        String INSERT_LOG = "INSERT INTO image_version (idimage, version) VALUES (?,?)";
        PreparedStatement ps;
        try {
//            Connection conn = DataSource.getConnection();
            ps = conn.prepareStatement(INSERT_LOG, Statement.RETURN_GENERATED_KEYS);
