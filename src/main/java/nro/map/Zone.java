...
//        synchronized (this) {
            ...
            if(p.chimFollow == (byte)1) {
                for (Player player : players) {
                    if (player != null && player.session != null && player.id != p.id) {
                        useDeTrungForMe(player, p, (byte) 7);
                    }
                }
            }
...
//        }
                    ...
                    if(itemPick.playerId == p.id || itemPick.playerId == -1){
                        if(itemPick.item.template.id == 76 || itemPick.item.template.id == 188 || itemPick.item.template.id == 189 || itemPick.item.template.id == 190) { //TRUONG HOP ITEM LA VANG
                            ...
                            if(p.noiTai.id != 0 && p.noiTai.idSkill == (byte)(-2)) {
                                rdVang += (int)(rdVang*Util.getPercentDouble(p.noiTai.param));
                            }
                            //END NOI TAI TANG VANG
                            p.vang = (Math.min(p.vang + rdVang, Constant.MAX_MONEY));
    ...
    public void PlayerDropItem(Player p, Item item) {
        try {
            ...
            m.writer().writeShort((short)itemMap.itemMapID);
            m.writer().writeShort(itemMap.itemTemplateID);
            m.writer().writeShort((short)itemMap.x);
            m.writer().writeShort((short)itemMap.y);
    ...
    //---
    public void VGo(Player p, @NotNull Message m, Map mapold) throws IOException {
        m.cleanup();
        ...
        synchronized (this) {
            ...
            try {
                ...
                if(map.MapNrNamec() && pl.nrNamec != 0) {
                    Server.gI().mapNrNamec[pl.nrNamec - 353] = pl.map.id;
                    Server.gI().nameNrNamec[(pl.nrNamec - 353)] = pl.map.template.name;
                    Server.gI().zoneNrNamec[(pl.nrNamec - 353)] = pl.zone.id;
                    Server.gI().pNrNamec[(pl.nrNamec - 353)] = pl.name;
                }
                if(!map.MapHome()) {
                    players.add(pl);
                    if(pl.petfucus == 1) {
...
//                    removeItemMapById(pl.id);
                    ...
                    msg.writer().writeShort((short)item.x);
                    msg.writer().writeShort((short)item.y);
                    msg.writer().writeInt(item.playerId);
//                    itemsMap.add(item);
                }
                else if((map.id == 42 || map.id == 43 || map.id == 44) && pl.taskId == (short)3 && pl.crrTask.index == (byte)1)
                { //NHIEM VU SAO BANG
...
//                        itemsMap.add(item);
                ...
                {
                    if(!this.map.MapSaoDen()) {
                        msg.writer().writeByte((byte)0);
                    } else {
                        msg.writer().writeByte((byte) itemsMap.size());
                        for (ItemMap itemMap : itemsMap) {
                ...
                // bg item
                if((map.id >= 153 && map.id < 166) || (map.id >= 122 && map.id <= 124) || (map.id >= 147 && map.id <= 148)) {
...
//                    itemsMap.add(item);
                    ...
                    if(!this.map.MapSaoDen()) {
                        ...
                        for (ItemMap itemMap : itemsMap) {
                            msg.writer().writeShort((short) itemMap.itemMapID);
                            msg.writer().writeShort(itemMap.itemTemplateID);
                            msg.writer().writeShort((short) itemMap.x);
                            msg.writer().writeShort((short) itemMap.y);
                            msg.writer().writeInt(itemMap.playerId);
                        }
                    }
                }
    ...
    public void upDownPointSaoDen(Player p) {
        if (map.MapSaoDen() && !p.isMapSaoDen) {
            ...
            if(p.xHPSaoDen > (byte)0) {
                p.hp = Math.min(p.hp, p.getHpFull());
    ...
    public void upDownPointMapCool(Player pl) {
        if (map.MapCold() && !pl.checkCTCold() && !pl.isDownPointCold) {
            ...
            pl.hp = Math.min(pl.hp, pl.getHpFull());
    ...
    public void upDownPointPETMapCool(Player pl) {
        if (map.MapCold() && !pl.detu.checkCTCold() && !pl.detu.isDownPointCold) {
            ...
            pl.detu.hp = Math.min(pl.detu.hp, pl.detu.getHpFull());
        ...
        try {
            for (Player player : players) {
                if (player != null && player.id != p.id) {
                    loadInfoPlayer(player.session, p);
                    //send chim cua minh cho nguoi khac
                    if (p.chimFollow == (byte) 1) {
                        useDeTrungForMe(player, p, (byte) 0);
                    }
                    if (p.petfucus == 1) {
                        loadInfoDeTu(player.session, p.detu);
                    }
                    if (p.pet2Follow == (byte) 1 && p.pet != null) {
                        loadInfoPet(player.session, p.pet);
                    }
                }
            }
        } catch (Exception e) {
...
//                msg.writer().writeShort(-1); // param eff
            ...
            msg.writer().writeByte((p.isMonkey ? (byte)1 : (byte)0)); //ismonkey
            ...
            for (Player player : players) {
                if (player != null && player.id != p.id) {
                    ...
                    if (p.imgNRSD > (byte) 0) {
                        Service.gI().updateBagNew(player.session, p.id, p.imgNRSD);
                        ClanService.gI().getBagBangNew(player.session, p.imgNRSD);
                    } else {
                        if (p.clan != null) {
                            //UPDATE BAG SAU LUNG
                            Service.gI().updateBagNew(player.session, p.id, p.clan.imgID);
                            //GET BAG SAU LUNG
                            ClanService.gI().getBagBangNew(player.session, p.clan.imgID);
                        }
                    }
                    //send chim cua minh cho nguoi khac
                    if (p.chimFollow == (byte) 1) {
    ...
    public Mob getMobByTempID(int id) {
        for (Mob mob : this.mobs) {
            if (mob != null && mob.template.tempId == id) {
                return mob;
            }
        }
        return null;
    ...
    // send message to me
    public void sendMessageToMe(Message m, @NotNull Player p) {
        p.session.sendMessage(m);
    }
    ...
    public void PlayerDead(@NotNull Player p) throws IOException {
        if (p.isdie == true) {
    ...
    public void FightMob(@NotNull Player p, @NotNull Message m) throws IOException {
                    ...
                    if (p.mp >= _manaSocola && !p.checkPlayerBiKhongChe() && !p.isdie && !mob.isDie && (_timeNOW - skillPlayerUse.lastTimeUseThisSkill > (long) (skillPlayerUse.coolDown))) {
                        ...
                        try {
                            ...
                            for (Player player : players) {
                                if (player != null) {
                                    player.session.sendMessage(m);
                                    ResetSocolaTask socolaTask = new ResetSocolaTask(player, mob);
                ...
                if (p.idSkillselect == 20 && p.mp >= skillPlayerUse.manaUse && !p.isdie && !mob.isDie && !p.checkPlayerBiKhongChe() && (_timeNOW - skillPlayerUse.lastTimeUseThisSkill > (long) (skillPlayerUse.coolDown))) {
                    ...
                    try {
                        ...
                        for (Player player : players) {
                            if (player != null) {
                                player.session.sendMessage(m);
                                ResetDCTTTask dcttTask = new ResetDCTTTask(player, mob);
                    ...
                    if (p.power >= 70000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 15);
                    } else if (p.power >= 60000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 11);
                    } else if (p.power >= 50000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 9);
                    } else if (p.power >= 40000000000L) {
                    ...
                    if (p.power >= 90000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 2);
                    }
                    if (p.power >= 100000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 2);
                    }
                    if (p.power >= 110000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 2);
                    }
                    if (p.power >= 120000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 2);
                    }
                    if (p.power >= 130000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 2);
                    }
                    if (p.power >= 140000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 2);
                    }
                    if (p.power >= 150000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 2);
                    }
                    if (p.power >= 160000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 2);
                    }
                    if (p.power >= 170000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 2);
                    }
                    if (p.power >= 180000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 5);
                    }
                    if (p.power >= 190000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 5);
                    }
                    if (p.power >= 200000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 5);
                    }
                    if (p.power >= 210000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 5);
                    }
                    if (p.power >= 220000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 5);
                    }
                    if (p.power >= 230000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 5);
                    }
                    if (p.power >= 240000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 5);
                    }
...
//                    }
                    ...
                    if(upSmTnDCTT > Constant.MAX_TNSM_HIT) {
                        upSmTnDCTT = Constant.MAX_TNSM_HIT;
                    }
                ...
                if(p.noMiss || skillPlayerUse.template.id == 1) {
                    p.noMiss = false;
                    miss = 7;
                }
                if (miss < 8) {
                ...
                if (p.mp >= kiUse && !p.isdie && !mob.isDie && mob.hp > 0 && !p.checkPlayerBiKhongChe() && (_timeNOW - skillPlayerUse.lastTimeUseThisSkill > (long) (skillPlayerUse.coolDown)) && (_timeNOW >= p.timeCanSkill)) {
                    ...
                    if (fatal || p.PEMCRIT > 0) {
                        ...
                        p.PEMCRIT = 0;
                        fatal = true;
                        //dame cai trang % sat thuong chi mang
                    ...
                    //CHECK DAME SET KICH HOAT
                    damage = Service.gI().getDameSetActive(p, skillPlayerUse, damage);
...
//                        }
                        ...
                        //GET TNSM
                        long upSmTn = Service.gI().getTnsmAdd(p, damage);
                        if (mob.template.tempId == 0) {
                            damage = 10;
                            upSmTn = 1;
                        }
                    ...
                    if (mob.isDie) {
                        ...
                        Service.gI().updateTaskFarmMob(p, mob);
                        //check TRUNG MABU
...
//                        }
                        ...
                        if (p.taskId == (short) 2) {
                            if ((p.gender == (byte) 0 && mob.template.tempId == TaskManager.gI().mobTASK0[p.taskId][p.crrTask.index]) ||
                                ...
                                ItemMap itemM = cNewItemMap(73, p.id, mob.pointX, mob.pointY);
                                if(itemM != null) {
                                    addItemToMap(itemM, p.id, mob.pointX, mob.pointY);
                                }
                            }
                        } else if (mob.isSocola > 0) {
                            ItemMap itemM = cNewItemMap(516, p.id, mob.pointX, mob.pointY);
                            if(itemM != null) {
                                addItemToMap(itemM, p.id, mob.pointX, mob.pointY);
                            }
                        } else {
                            if (mob.template.tempId != 0) {
                                int percentDrop = Util.nextInt(0, Constant.RANGE_DROP_NORMAL);
                                int percentDropItemGod = Util.nextInt(0, Constant.RANGE_ITEM_GOD);
                                int percentDropKichHoat = Util.nextInt(0, Constant.RANGE_ITEM_ACTIVE);
                                int perTA = Util.nextInt(0, Constant.RANGE_THUC_AN);
                                if (percentDropKichHoat < Constant.RATE_ITEM_ACTIVE && map.MapSetKichHoat())
                                { //ITEM ROT LA ITEM KICH HOAT
                                    int id = Util.nextInt(0, 5); //RANDOM TU 0,1,2,3,4
                                    ItemSell sellKichHoat = ItemSell.getItemSell(Constant.ITEM_ACTIVE[p.gender][id], (byte) 0);
                                    ...
                                    if(itemsMap.size() > 0) {
                                        itemROI.itemMapID = (itemsMap.get(itemsMap.size() - 1).itemMapID + 1);
                                        if(itemROI.itemMapID >= 50000) {
                                            itemROI.itemMapID = 0;
                                        }
                                    } else {
                                        itemROI.itemMapID = 0;
...
//                                    itemROI.itemMapID = itemsMap.size();
                                    itemROI.itemTemplateID = (short) (Constant.ITEM_ACTIVE[p.gender][id]);
                                    ...
                                    if(p.gender == (byte)1) {
                                        rdOptionKichHoat = Util.nextInt(0, 4);
                                    }
                                    _ITEMKICHHOAT.itemOptions.add(new ItemOption(Constant.ITEM_ACTIVE_OPTION[p.gender][rdOptionKichHoat], 0));
                                    if(p.gender == (byte)1) {
                                        _ITEMKICHHOAT.itemOptions.add(new ItemOption(Constant.ITEM_ACTIVE_OPTION[p.gender][rdOptionKichHoat + 4], 0));
                                    } else {
                                        _ITEMKICHHOAT.itemOptions.add(new ItemOption(Constant.ITEM_ACTIVE_OPTION[p.gender][rdOptionKichHoat + 3], 0));
                                    }
...
//                                    itemDrop.add(itemROI);
                                    if(itemROI != null) {
                                        addItemToMap(itemROI, p.id, mob.pointX, mob.pointY);
                                        //CHECK BUA THU HUT
                                        if(p.getBuaThuHut()) {
                                            PickItemDrop(p, (short)itemROI.itemMapID);
                                        }
                                    }
//                                    itemsMap.addAll(itemDrop);
                                }
                                else if (percentDrop < Constant.RATE_DROP_NORMAL && map.MapCell() && p.useMayDoCapsule)
                                { //ITEM ROT KHI DUNG RADA DO CAPSULE
                                    int id = Util.nextInt(0, 35);
                                    if (id >= 20) {
                                        id = 34;
                                    }
                                    Item itemMap = ItemSell.getItemNotSell(Constant.ITEM_DROP_2[id]);
                                    ItemMap item = new ItemMap();
                                    item.playerId = p.id;
                                    item.x = mob.pointX;
                                    item.y = mob.pointY;
                                    if(itemsMap.size() > 0) {
                                        item.itemMapID = (itemsMap.get(itemsMap.size() - 1).itemMapID + 1);
                                        if(item.itemMapID >= 50000) {
                                            item.itemMapID = 0;
                                        }
                                    } else {
                                        item.itemMapID = 0;
...
//                                    item.itemMapID = itemsMap.size();
                                    item.itemTemplateID = (short) Constant.ITEM_DROP_2[id];
                                    itemMap.template = ItemTemplate.ItemTemplateID(Constant.ITEM_DROP_2[id]);
                                    item.item = itemMap;
                                    if(item != null) {
                                        addItemToMap(item, p.id, mob.pointX, mob.pointY);
                                        //CHECK BUA THU HUT
                                        if(p.getBuaThuHut()) {
                                            PickItemDrop(p, (short)item.itemMapID);
                                        }
                                    }
//                                    itemDrop.add(item);
//                                    itemsMap.addAll(itemDrop);
                                ...
                                else if (percentDropItemGod < Constant.RATE_ITEM_GOD && map.MapCold())
                                {
                                    Item itemGOD = null;
                                    int _ITEMMAPID = -1;
                                    percentDropItemGod = Util.nextInt(0, 15);
                                    itemGOD = ItemSell.getItemNotSell(Constant.ITEM_GOD[percentDropItemGod]);

                                    _ITEMMAPID = Constant.ITEM_GOD[percentDropItemGod];
...
//                                        itemROI.itemMapID = itemsMap.size();
                                        ...
                                        Item _ITEMGOD = new Item(itemGOD);
...
//                                else if (perTA < Constant.RATE_THUC_AN && p.checkFullSetThan() && map.MapCold()) {
//                                    perTA = Util.nextInt(0, 5);
//
//                                    ItemMap itemM = cNewItemMap(Constant.ITEM_THUC_AN[perTA], p.id, mob.pointX, mob.pointY);
//                                    if(itemM != null) {
//                                        addItemToMap(itemM, p.id, mob.pointX, mob.pointY);
//                                        //CHECK BUA THU HUT
//                                        if(p.getBuaThuHut()) {
//                                            PickItemDrop(p, (short)itemM.itemMapID);
//                                        }
//                                    }
//                                } else if (perTA < Constant.RATE_ANGEL && map.MapFarmThienSu() && p.checkSetByLevel((byte) 14)) {
//                                    perTA = Util.nextInt(0, 5);
//
//                                    ItemMap itemM = cNewItemMap(Constant.ITEM_ANGEL[perTA], p.id, mob.pointX, mob.pointY);
//                                    if(itemM != null) {
//                                        addItemToMap(itemM, p.id, mob.pointX, mob.pointY);
//                                        //CHECK BUA THU HUT
//                                        if(p.getBuaThuHut()) {
...
//                                    }
//                                }
                                else if (perTA < Constant.RATE_UP_PORATA && map.MapThanhDia())
                                {
                                    ...
                                    if (id >= 20) {
                                        ...
                                        id = Constant.ITEM_DROP[id];
                                    }

                                    ItemMap itemM = cNewItemMap(id, p.id, mob.pointX, mob.pointY);
                                    if(itemM != null) {
                                        addItemToMap(itemM, p.id, mob.pointX, mob.pointY);
                                        //CHECK BUA THU HUT
                                        if(p.getBuaThuHut()) {
                                ...
                                else if (percentDrop < Constant.RATE_DROP_NORMAL)
                                {
                                    ...
                                    ItemMap itemM = cNewItemMap(Constant.ITEM_DROP[id], p.id, mob.pointX, mob.pointY);
                                    if(itemM != null) {
                                        addItemToMap(itemM, p.id, mob.pointX, mob.pointY);
                                        //CHECK BUA THU HUT
                                        if(p.getBuaThuHut()) {
                                    ...
                                    Item itemMap = ItemSell.getItemNotSell(Constant.ITEM_DROP[id]);
//                                    ItemMap item = new ItemMap();
//                                    item.playerId = p.id;
//                                    item.x = mob.pointX;
//                                    item.y = mob.pointY;
//                                    item.itemMapID = itemsMap.size();
//                                    item.itemTemplateID = (short)Constant.ITEM_DROP[id];
//                                    itemMap.template = ItemTemplate.ItemTemplateID(Constant.ITEM_DROP[id]);
//                                    item.item = itemMap;
//                                    itemDrop.add(item);
//                                    itemsMap.addAll(itemDrop);
                    ...
                    if (p.chimFollow == (byte) 1 && !mob.isDie && p.dameChim > 0) {
                        ...
                        if (p.getSetKichHoatFull() == (byte) 6) {
                            damage = (int)(damage*Util.getPercentDouble(200 + p.getDameUpItemActive()));
                        }
...
//                            }
                            ...
                            int percentDrop2 = Util.nextInt(0, Constant.RANGE_DROP_NORMAL);
                            if (mob.template.tempId != 0 && percentDrop2 < Constant.RATE_DROP_NORMAL) {
    ...
    public void sendHutHP(@NotNull Player _player, int _damage, boolean isAttackMob) {
        int dameHutHp = 0;
        if(isAttackMob) {
            dameHutHp = (int)(Util.getPercentDouble((int)_player.getPercentHutHp())*_damage);
        } else if(_player.ItemBody[5] != null && _player.ItemBody[5].template.id == 448) {
    ...
    public void postUpdateHP(@NotNull Player _player) {
        try {
    ...
    public void postUpdateKI(@NotNull Player _player) {
        try {
            Message ms = new Message(-30);
    ...
    public void sendHutKI(@NotNull Player _player, int _damage) {
    ...
    public void sendMonkeyAttack(@NotNull Player pl, Mob mob, int damage) {
        Message m = null;
        try {
            for (Player player : players) {
                if (player != null) {
    ...
    public void loadPlayers(@NotNull Player p) {
        try {
            for (Boss b : bossMap) {
                loadInfoBoss(b);
            }
            for (Player player : players) {
                if (player != null) {
                    if (p != player) {
                        loadInfoPlayer(p.session, player);
                        if (player.chimFollow == (byte) 1) {
                            useDeTrungForMe(p, player, (byte) 0);
                        }
                    }

                    if (player.cPk != 0) {
                        Service.gI().changeFlagPK(player, player.cPk);
...
//            msg.writer().writeShort(-1); // param eff
    ...
    public void FightBoss(@NotNull Player p, int bossId) throws IOException {
        int damage = 0;
        Message m = null;
...
//        Skill skillPlayerUse = skilldata.getSkillBySkillTemplate(p.gender, skillUse.skillId, skillUse.point);
        ...
        //CHECK TAM DANH CUA SKILL
        if (Service.gI().checkCanAttackBoss(_bTarget) && !p.isTroi) {
...
//                    _bTarget.typePk = 1;
                    ...
                    if(_bTarget._typeBoss != 1 && _bTarget._typeBoss != 2) { //Broly khong roi do
                        ...
                        } else if(_bTarget._typeBoss == (byte)51) {
                            if(Util.nextInt(0, 100) == 75) {
                                ItemMap itemM = dropItemGodCereal(p, _bTarget.x, _bTarget.y);
                                if(itemM != null) {
                                    addItemToMap(itemM, p.id, _bTarget.x, _bTarget.y);
                                }
                            }
                        } else if(_bTarget._typeBoss >= (byte)52 && _bTarget._typeBoss <= (byte)55) {
                            if(Util.nextInt(0, 100) >= 97) {
                                ItemMap itemM = cNewItemMap(Util.nextInt(1201, 1203), p.id, _bTarget.x, _bTarget.y);
                                if(itemM != null) {
                                    addItemToMap(itemM, p.id, _bTarget.x, _bTarget.y);
                                }
                            }
                        ...
                        //ALL BOSS ROT DO
                        if(_bTarget._typeBoss == (byte)4 || _bTarget._typeBoss == (byte)6 ||
                                (_bTarget._typeBoss >= (byte)18 && _bTarget._typeBoss <= (byte)30 && _bTarget._typeBoss != (byte)29)
                                || _bTarget._typeBoss == (byte)48) {
                            ItemMap itemROT = dropItemGOD(p, _bTarget.x, _bTarget.y, _bTarget._typeBoss);
                            if(itemROT != null) {
                                addItemToMap(itemROT, p.id, _bTarget.x, _bTarget.y);
                            }
                        }
                    }
                    if(_bTarget._typeBoss == 2 && (p.havePet == 0 || (p.havePet == 1 && !p.detu.isMabu))) { //boss die la super broly
                    ...
                    if(_bTarget._typeBoss < (byte)44 || _bTarget._typeBoss > (byte)47) {
                        for (Player player : players) {
                            if (player != null) {
                                sendDieToAnotherPlayer(player, _bTarget);
                            }
                        }
                    }
                    //CHECK NHIEM VU SAN BOSS
                    Service.gI().updateTaskKillBoss(p, _bTarget);
                    //END NHIEM VU SAN BOSS

                    //DELETE BOSS
//                    timerDeleteBoss(_bTarget);
    //                MobStartDie(p, damage, mob, fatal, itemDrop);
                } else {
                    dameChar(_bTarget.id, _bTarget.hp, damageDCTT, true);
                    //dame hut mau
            ...
            if(p.mp >= kiUse && !p.isdie && !_bTarget.isdie && _bTarget.hp > 0 && _bTarget.typePk == 5 && !p.checkPlayerBiKhongChe() && (_timeNOW - skillPlayerUse.lastTimeUseThisSkill > (long)(skillPlayerUse.coolDown)) && (_timeNOW >= p.timeCanSkill)) {
                //CHECK NOI TAI LAZE CO GIAM THOI GIAN HOI CHIEU
                if(p.noiTai.id != 0 && skillPlayerUse.template.id == p.noiTai.idSkill && p.idSkillselect == 11)
                { //LAZE
                    long _timeDOWN = (long)(Util.getPercentDouble((int)(p.noiTai.param))*skillPlayerUse.coolDown);
                    skillPlayerUse.lastTimeUseThisSkill = _timeNOW - _timeDOWN;
                    //SEND TIME DOWN NOI TAI
                ...
                if(p.upDameAfterNoiTai && p.noiTai.id != 0) {//% DAME DON KE TIEP
                    p.upDameAfterNoiTai = false;
//                    if((p.gender == (byte)0 && _bTarget.isDctt) || p.gender != (byte)0) {
//                        _bTarget.isDctt = false;
                        damage += (int) (damage * Util.getPercentDouble((int) p.noiTai.param));
//                    }
                } else if (skillPlayerUse.template.id == p.noiTai.idSkill) { //%DAM CHIEU DAM VA CHUONG
                    damage += (int)(damage*Util.getPercentDouble((int)p.noiTai.param));
                ...
                if(p.ItemBody[5] != null && (p.ItemBody[5].template.id == 710 || p.ItemBody[5].template.id == 711) && !p.isChuongX3X4 && (System.currentTimeMillis() >= p.timeX3X4)) {
                    int perX3X4 = Util.nextInt(0, 100);
                    if(perX3X4 >= 52 && perX3X4 <= 56) {
                        p.isChuongX3X4 = true;
                        p.timeX3X4 = System.currentTimeMillis() + 60000;
                        if(p.ItemBody[5].template.id == 710) {
                            damage = damage*3;
...
//                    _bTarget.typePk = 1;
                    ...
                    if(_bTarget._typeBoss != 1 && _bTarget._typeBoss != 2) { //Broly khong roi do
                        ...
                        //ALL BOSS ROT DO
                        if(_bTarget._typeBoss == (byte)4 || _bTarget._typeBoss == (byte)6 || (_bTarget._typeBoss >= (byte)18 && _bTarget._typeBoss <= (byte)30 && _bTarget._typeBoss != (byte)29) || _bTarget._typeBoss == (byte)48) {
                            ItemMap itemROT = dropItemGOD(p, _bTarget.x, _bTarget.y, _bTarget._typeBoss);
                            if(itemROT != null) {
                                addItemToMap(itemROT, p.id, _bTarget.x, _bTarget.y);
                            }
...
//                        timerDeleteBoss(_bTarget);
    ...
    public void FightChar(@NotNull Player p, int charId) throws IOException {
        int damage = 0;
        int damageChim = 0;
        Message m = null;
...
//            ySkill = (short)(ySkill + 20);
        ...
        //CHECK TAM DANH CUA SKILL
        if(Service.gI().checkCanAttackChar(p, _bTarget) && !p.isTroi) {
...
//                        }
        ...
        //                drawSkillVip(p, mob, skillPlayerUse.skillId, damage);
                    ...
                    if(p.chimFollow == (byte)1 && !_bTarget.isdie && p.dameChim > 0) {
                        ...
                        _bTarget.hasInjured(damageChim);
    ...
    //FIGHT DE TU
    public void FightDetu(@NotNull Player p, int charId) throws IOException {
        int damage = 0;
        int damageChim = 0;
        Message m = null;
        ...
        //CHECK TAM DANH CUA SKILL
        if(Service.gI().checkCanAttackDeTu(p, _bTarget) && !p.isTroi) {
...
//            }
    ...
    public void playerMove(Player _player) {
        ...
        try {
            for (Player player : players) {
                if (player != null && player.id != _player.id) {
                    msg = new Message(-7);
                    msg.writer().writeInt(_player.id);
                    msg.writer().writeShort(_player.x);
                    msg.writer().writeShort(_player.y);
    ...
    public void chatTG(@NotNull Session _session, String _text) {
...
//        Map map = player.map;
    ...
    public void selectUIZone(Player p, @NotNull Message m) throws IOException {
        ...
        if(Server.gI().supportNV && (((p.map.id - 1) == Server.gI().mapTDST && zoneid == (byte)(Server.gI().khuTDST) && p.taskId != (short)22) ||
                (p.map.id == 80 && zoneid == (byte)(Server.gI().khuFide) && p.taskId != (short)23) ||
                ((p.map.id - 1) == Server.gI().map1920 && zoneid == (byte)(Server.gI().khu1920) && p.taskId != (short)25) ||
                (p.map.id == 104 && zoneid == (byte)(Server.gI().khu151413) && p.taskId != (short)26) )) {
...
//                            Service.gI().loadPoint(_session, player);
    ...
    public void useKenhKhi(@NotNull Player player) {
...
//        Skill templateSkillUse = skilldata.getSkillBySkillTemplate(player.gender, skillUse.skillId, skillUse.point);
        ...
        if(player.mp >= _manaUse && !player.checkPlayerBiKhongChe() && ((_TIMENOW - templateSkillUse.lastTimeUseThisSkill) > (long)templateSkillUse.coolDown)) {
            ...
            if(player.getSetKichHoatFull() == (byte)2) {
                dameKenhKhi = (int)(dameKenhKhi*Util.getPercentDouble(200 + player.getDameUpItemActive()));
            }
                    ...
                    for(Mob mob: mobs) {
                        if (Math.abs(player.x - mob.pointX) < templateSkillUse.dx && Math.abs(player.y - mob.pointY) < templateSkillUse.dy && !mob.isDie) {
                                ...
                                if(mob.isDie) {
                                    ...
                                    if(mob.template.tempId != 0) { //TODO
                                        int percentDrop = Util.nextInt(0, 10);
                                        if(percentDrop < 3) {
                                            int id = Util.nextInt(0, Constant.ITEM_DROP.length - 2);

                                            ItemMap itemM = cNewItemMap(Constant.ITEM_DROP[id], player.id, mob.pointX, mob.pointY);
                                            if(itemM != null) {
                                                addItemToMap(itemM, player.id, mob.pointX, mob.pointY);
                                                //CHECK BUA THU HUT
                                                if(player.getBuaThuHut()) {
                                                    try {
                                                        PickItemDrop(player, (short)itemM.itemMapID);
                                                    } catch (IOException e) {
...
//                                        }
                    ...
                    if(bossMap.size() > 0) {
                        for (Boss boss : bossMap) {
                            if (Math.abs(player.x - boss.x) < templateSkillUse.dx && Math.abs(player.y - boss.y) < templateSkillUse.dy && !boss.isdie && Service.gI().checkCanAttackBoss(boss)) {
                                ...
                                if (boss.isdie && boss.typePk == 5) {
//                                    boss.typePk = 1;
                                    //send dame
                                    dameChar(boss.id, boss.hp, dameBoom, false);
                                    //REMOVE ALL KHONG CHE KHI BOSS CHET
...
//                                                try {
//                                                    m = new Message(68);
//                                                    m.writer().writeShort((short) itemROI.itemMapID);
//                                                    m.writer().writeShort(itemROI.item.template.id);
//                                                    m.writer().writeShort(boss.x);
//                                                    m.writer().writeShort(boss.y);
//                                                    m.writer().writeInt(player.id);
//                                                    m.writer().flush();
...
//                                            }
                                            ...
                                            ItemMap itemM = newItemMAP(itemDT, player.id, boss.x, boss.y);
                                            if (itemM != null) {
                                                addItemToMap(itemM, player.id, boss.x, boss.y);
                                            }
                                        } else if (boss._typeBoss >= (byte) 26 && boss._typeBoss <= (byte) 28) {//cell rot 3s
                                            if (Util.nextInt(0, 2) == 0) {
                                                ItemMap itemM = newItemMAP(16, player.id, boss.x, boss.y);
                                                if (itemM != null) {
                                                    addItemToMap(itemM, player.id, boss.x, boss.y);
                                                }
                                            }
                                        } else if (boss._typeBoss == (byte) 30) {//sieu bo hung rot 2s
                                            if (Util.nextInt(0, 2) == 0) {
                                                ItemMap itemM = newItemMAP(15, player.id, boss.x, boss.y);
                                                if (itemM != null) {
                                                    addItemToMap(itemM, player.id, boss.x, boss.y);
                                                }
                                            }
                                        } else if (boss._typeBoss == (byte) 48) {//zamasu rot da bao ve
                                            if (Util.nextInt(0, 10) <= 0) {
                                                ItemMap itemM = newItemMAP(987, player.id, boss.x, boss.y);
                                                if (itemM != null) {
                                                    addItemToMap(itemM, player.id, boss.x, boss.y);
                                                }
                                            }
                                            ...
                                            if(Util.nextInt(0, 100) == 75) {
                                                ItemMap itemM = dropItemGodCereal(player, boss.x, boss.y);
                                                if(itemM != null) {
                                                    addItemToMap(itemM, player.id, boss.x, boss.y);
                                                }
                                            }
                                            ...
                                            if(Util.nextInt(0, 100) >= 97) {
                                                ItemMap itemM = cNewItemMap(Util.nextInt(1201, 1203), player.id, boss.x, boss.y);
                                                if(itemM != null) {
                                                    addItemToMap(itemM, player.id, boss.x, boss.y);
                                                }
                                            }
...
//                                        }
                                        ...
                                        if (boss._typeBoss == (byte) 4 || boss._typeBoss == (byte) 6 || (boss._typeBoss >= (byte) 18 && boss._typeBoss <= (byte) 30 && boss._typeBoss != (byte) 29) || boss._typeBoss == (byte)48) {
                                            ItemMap itemROT = dropItemGOD(player, boss.x, boss.y, boss._typeBoss);
                                            if (itemROT != null) {
                                                addItemToMap(itemROT, player.id, boss.x, boss.y);
                                            }
                                        }
...
//                                        }
                                    ...
                                    if (boss._typeBoss < (byte) 44 || boss._typeBoss > 47) {
                                        for (Player _pp : players) {
                                            sendDieToAnotherPlayer(_pp, boss);
                                        }
                                    }
                                    //CHECK NHIEM VU SAN BOSS
...
//                                    timerDeleteBoss(_bossDIE);
                                ...
                                attachedChar(player.id, boss.id, templateSkillUse.skillId);
                            }
                        }
                    }
    ...
    public void useLaze(@NotNull Player player) {
...
//            player.mp = 0;
    ...
    public void useHuytSao(@NotNull Player player) {
...
//            m.writer().writeInt(hp);
    ...
    public void useProtect(@NotNull Player player) {
...
//        }
    ...
    public void useProtectPet(@NotNull Player player) {
        Skill skillUse = player.getSkillById(19);
        SkillData skilldata = new SkillData();
...
//        }
    ...
    //tdhs td
    public void useTDHS(@NotNull Player player) {
...
//                }
    ...
    public void useTDHSPet(@NotNull Player player) {
        Skill skillUse = player.getSkillById(6);
        SkillData skilldata = new SkillData();
...
//                        arrIdBoss[countChar] = _boss.id;
    ...
    public void useDeTrungForMe(Player me, @NotNull Player player, byte type) {
        Skill templateSkillUse = player.getSkillByIDTemplate((short)12);
        if(templateSkillUse != null) {
            Message m = null;
            try {
    ...
    public void useDeTrung2(@NotNull Player player) {
...
//        Skill templateSkillUse = skilldata.getSkillBySkillTemplate(player.gender, skillUse.skillId, skillUse.point);
    ...
    public void PetAttack(Player _player, @NotNull Detu _detu, byte type) {
...
//                                                    }
    ...
    public void PetSendAttack(Player _player, Detu _detu, Mob _mob, Skill _skill) throws IOException {
        ...
        if(_detu.getSetKichHoatFull() == (byte)3 && _skill.template.id == (byte)1) {
            damage = (int)(damage*Util.getPercentDouble(200 + _player.getDameUpItemActive()));
        ...
        //CHECK BUA MANH ME DE TU MABU
        if (_player.detu.isMabu && _player.getBuaSucManhMabu()) {
            damage = (int) (damage * 1.5);
        }

        long upSmTn = (long)(damage*2);
//        long upSmTn = (long)(damage*20);
        if(_player.getBuaDetux3()) {
            upSmTn = damage*3;
        }
        upSmTn = _detu.getPercentUpTnSm(upSmTn); //get tnsm spl tnsm

        //CHECK BUA TRI TUE MABU
        if (_player.detu.isMabu && _player.getBuaTriTueMabu()) {
        ...
        if (_detu.power >= 70000000000L) {
            upSmTn = (long) (upSmTn / 15);
        } else if (_detu.power >= 60000000000L) {
            upSmTn = (long) (upSmTn / 11);
        } else if (_detu.power >= 50000000000L) {
            upSmTn = (long) (upSmTn / 9);
        } else if (_detu.power >= 40000000000L) {
        ...
        if(upSmTn > Constant.MAX_TNSM_HIT) {
            upSmTn = Constant.MAX_TNSM_HIT;
        }
        if(upSmTn > 0) {
...
//                openSkillPetAndUpPoint(_detu);
            ...
            if(_player.power < 180000000000L && _player.power < (_player.getPowerLimit()*1000000000L)) {
...
//        if(_detu.tiemNang > 0 && (_detu.hpGoc < _detu.getHpMpLimit() || _detu.mpGoc < _detu.getHpMpLimit() || _detu.damGoc < _detu.getDamLimit())) {
        ...
        if (_mob.isDie) {
            ArrayList<ItemMap> itemDrop = new ArrayList<>();
            if(_mob.template.tempId != 0) {
                int idItemNotSell[] = {1203, 1204, 1205, 1206, 1207,17,188,189,190,18,19,20, 441,442,443,444,445,446,447, 17,188,189,190, 225, 76,188,189,190,18,19,20,17,188,189,190,18,19,20};
                int percentDrop = Util.nextInt(0, Constant.RANGE_DROP_NORMAL);
                int percentDropItemGod = Util.nextInt(0, Constant.RANGE_ITEM_GOD);
                int perTA = Util.nextInt(0, Constant.RANGE_THUC_AN);
                if(percentDropItemGod < Constant.RATE_ITEM_GOD && map.MapCold()) {
                    Item itemGOD = null;
                    int _ITEMMAPID = -1;
                    int idItemGod[] = {555, 556, 561, 562, 563, 557, 558, 561, 564, 565, 559, 560, 561, 566, 567};
                    percentDropItemGod = Util.nextInt(0, 15);
                    itemGOD = ItemSell.getItemNotSell(idItemGod[percentDropItemGod]);
...
//                else if(perTA < Constant.RATE_THUC_AN && _detu.checkFullSetThan() && map.MapCold()) {
...
//                    itemDrop.add(item);
//                    itemsMap.addAll(itemDrop);
//                } else if(perTA < Constant.RATE_ANGEL && map.MapFarmThienSu() && _detu.checkSetByLevel((byte)14)) {
...
//        }
    ...
    public void openSkillPetAndUpPoint(@NotNull Detu _detu) {
        int rdSkill2 = 0;
    ...
    public void pointUpForPet(@NotNull Detu _detu) {
...
//            System.out.println("Tong tiem nang la: " + _detu.tiemNang);
            //CHECK 70% TNSM co du de tang giap khong
//            long tempUse = (long)(_detu.tiemNang * 0.7);
//            (this.defGoc + 5) * 100000;
//            (tempUse / 100000)
...
//            }
            ...
            msg.writer().writeBoolean(false);  //co dang vo hinh hay khong
            msg.writer().writeByte((byte)_boss.typePk);  //type pk
                        ...
                        if((timeNow - _broly.lastTimeUseChargeSkill) > 45000 && _broly.hp < _broly.hpFull && !_broly.isTTNL) {
                            ...
                            if(_broly.hp <= (int)(_broly.hpFull*0.5) && _broly._typeBoss == 1) { //duoi 0.5 thi moi len hp
                                if(_broly.hp <= (int)(_broly.hpFull*0.1)) {
                                    _broly.hpFull = Math.min((int) (_broly.hpFull * 1.6), 2000000000);
                                } else {
                                    _broly.hpFull = Math.min((int) (_broly.hpFull * 1.4), 2000000000);
                                }
...
//                        players.get(_index).timerStone = timerEndStone;
    ...
    public void bossAttackChar(@NotNull Boss _boss,@NotNull Player _bTarget) {
        SkillData skilldata = new SkillData();
...
//                Util.log("TI LE NE DON: " + _bTarget.getPercentNedon());
            ...
            if(!_boss.isCharFreez && !_boss.isCharSleep && !_boss.isCharDCTT && !_boss.isCharBlind) {
                ...
                if(_bTarget.hp <= 0) {
                    ...
                    if(damePST > 0) {
                        damePST = (int)(damage*Util.getPercentDouble(damePST));
                        if (_boss._typeBoss == (byte)2) {
                            damePST = damePST / 5;
                        }
                        _boss.hp = Math.max((_boss.hp - damePST), 1);
                        //send dame
    ...
    public void resetTROIKHIMOVE(@NotNull Player p) {
        Message m = null;
        p.isTroi = false;
        if(p.MOBHOLD != null) {
    ...
    //                        haveBROLY = false;
                            ...
                            else if(_BOSS._typeBoss == 6)
                            { // SUPER BLACK GOKU DIE
                            ...
                            else if(_BOSS._typeBoss == (byte)8)
                            { //MAP DAU DINH DIE
                            ...
                            else if(_BOSS._typeBoss == (byte)9)
                            { //RAMBO DIE
                            ...
                            else if(_BOSS._typeBoss == (byte)10)
                            {
                            ...
                            else if(_BOSS._typeBoss == (byte)11)
                            {
                            ...
                            else if(_BOSS._typeBoss == (byte)13)
                            {
                            ...
                            else if(_BOSS._typeBoss == (byte)12)
                            {
                            ...
                            else if(_BOSS._typeBoss == (byte)14)
                            { //TIEU DOI TRUONG DIE
                            ...
                            else if(_BOSS._typeBoss == (byte)15)
                            { //FIDE 1 DIE
                            ...
                            else if(_BOSS._typeBoss == (byte)16)
                            { //FIDE 2 DIE
                                Timer timerFide = new Timer();
                                TimerTask ttFide = new TimerTask() {
                                    public void run()
                            ...
                            else if(_BOSS._typeBoss == (byte)17)
                            { //FIDE 3 DIE
                                Timer timerFide = new Timer();
                                TimerTask ttFide = new TimerTask() {
                                    public void run() {
                            ...
                            else if(_BOSS._typeBoss == (byte)18)
                            {
                                //REMOVE SO 20 VA INIT LAI
                                Boss B20 = getBossByID(-200136);
                                leaveBoss(B20);
                                B20 = new Boss(136, (byte) 19, xDIE, yDIE);
                                B20.typePk = (byte) 5;
                                bossMap.add(B20);
                                loadBossNoPet(B20);
                            }
                            else if(_BOSS._typeBoss == (byte)19)
                            {
...
//                                }
//                            }
                            else if(_BOSS._typeBoss == (byte)20)
                            {
                                //REMOVE SO 14 VA INIT LAI
                                Boss B14 = getBossByID(-200138);
                                leaveBoss(B14);
                                B14 = new Boss(138, (byte)21, xDIE, yDIE);
                                B14.typePk = (byte)5;
                                bossMap.add(B14);
                                loadBossNoPet(B14);
                            }
                            else if(_BOSS._typeBoss == (byte)21)
                            {
                                //REMOVE SO 13 VA INIT LAI
                                Boss B13 = getBossByID(-200139);
                                leaveBoss(B13);
                                B13 = new Boss(139, (byte)22, xDIE, yDIE);
                                B13.typePk = (byte)5;
                                bossMap.add(B13);
                                loadBossNoPet(B13);
                            }
                            else if(_BOSS._typeBoss == (byte)22)
                            {
...
//                                }
//                            }
                            else if(_BOSS._typeBoss == (byte)23)
                            ...
                            else if(_BOSS._typeBoss == (byte)24)
                            { //PIC  //REMOVE KING KONG VA INIT LAI
                            ...
                            else if(_BOSS._typeBoss == (byte)25)
                            { //KING KONG
                            ...
                            else if(_BOSS._typeBoss == (byte)26)
                            { // Xen 1 DIE
                            ...
                            else if(_BOSS._typeBoss == (byte)27)
                            { // Xen 2 DIE
                            ...
                            else if(_BOSS._typeBoss == (byte)28)
                            { // XEN HOAN THIEN DIE
                                if(_BOSS.id == -200146) {
                            ...
                            else if(_BOSS._typeBoss == (byte)36)
                            { //DRABULA
                            ...
                            else if(_BOSS._typeBoss == (byte)37)
                            { //BUI BUI
                                int idDIE = Math.abs(_BOSS.id + 200000);
                            ...
                            else if(_BOSS._typeBoss == (byte)38)
                            { //YACON
                                int idDIE = Math.abs(_BOSS.id + 200000);
                            ...
                            else if(_BOSS._typeBoss == (byte)39)
                            { //MABU
                                int idDIE = Math.abs(_BOSS.id + 200000);
    ...
    //                            timerMabu.schedule(ttMabu, 0, 1000);
                            ...
                            else if(_BOSS._typeBoss == (byte)40)
                            { // CHILLED DIE
                            ...
                            else if(_BOSS._typeBoss == (byte)41)
                            { //CHILLED 2 DIE
                            ...
                            else if(_BOSS._typeBoss == (byte)42)
                            { // LYCHEE DIE
                                int lvG = _BOSS.lvGas;
...
//                                itemM.y = yDIE;
//                                if(itemsMap.size() > 0) {
//                                    itemM.itemMapID = (itemsMap.get(itemsMap.size() - 1).itemMapID + 1);
//                                    if(itemM.itemMapID >= 50000) {
//                                        itemM.itemMapID = 0;
//                                    }
//                                } else {
//                                    itemM.itemMapID = 0;
...
//                                addItemToMap(itemM, -1, (short)768, (short)480);
                            ...
                            else if(_BOSS._typeBoss == (byte)43)
                            { //END KHI GA
                                //ROT CAI TRANG
...
//                                addItemToMap(itemM, -1, (short)768, (short)480);
                            ...
                            else if(_BOSS._typeBoss == (byte)51)
                            {
                                Timer timerHeeter = new Timer();
                                TimerTask tt15 = new TimerTask() {
                                    public void run()
                                    {
                                        Service.gI().initHeeter();
                                        timerHeeter.cancel();
                                    };
                                };
                                timerHeeter.schedule(tt15, 15000);
                            }
                            else if(_BOSS._typeBoss == (byte)52)
                            {
                                Timer timerTDST = new Timer();
                                TimerTask ttTDST = new TimerTask() {
                                    public void run()
                                ...
                                timerTDST.schedule(ttTDST, 1800000);
                            }
                            else if(_BOSS._typeBoss == (byte)53)
                            {
                                Boss _So1 = getBossByID((int)(-200702));
                                leaveBoss(_So1);
                                _So1 = new Boss(702, (byte)54, xDIE, yDIE);
                                _So1.typePk = (byte)5;
                                bossMap.add(_So1);
                                loadBossNoPet(_So1);
                            }
                            else if(_BOSS._typeBoss == (byte)54)
                            {
                                Boss _So2 = getBossByID((int)(-200703));
                                leaveBoss(_So2);
                                _So2 = new Boss(703, (byte)55, xDIE, yDIE);
                                _So2.typePk = (byte)5;
                                bossMap.add(_So2);
                                loadBossNoPet(_So2);
                            }
                            else if(_BOSS._typeBoss == (byte)55)
                            {
                                Boss _So3 = getBossByID(-200704);
                                leaveBoss(_So3);
                                _So3 = new Boss(704, (byte)52, xDIE, yDIE);
                                _So3.typePk = (byte)5;
                                bossMap.add(_So3);
                                loadBossNoPet(_So3);
                            }
                        ...
                        if (_BOSS._typeBoss < (byte) 7 || (_BOSS._typeBoss > (byte) 18 && _BOSS._typeBoss != (byte) 29 && _BOSS._typeBoss < (byte) 36)
                                || (_BOSS._typeBoss >= (byte) 40 && _BOSS._typeBoss <= (byte) 41) || _BOSS._typeBoss >= (byte) 48) {
                            long timeNow = System.currentTimeMillis();
                            if ((timeNow - _BOSS.lastTimeUseChargeSkill) > 45000 && _BOSS.hp < _BOSS.hpFull && !_BOSS.isTTNL && !_BOSS.checkBiKhongChe()) {
                                ...
                                TimerTask petTTNL = new TimerTask() {
                                    public void run() {
                                        if ((long) _BOSS.hp >= (hpBossNow + (long) ((_BOSS.hpFull) * 0.1)) || _BOSS.hp >= _BOSS.hpFull || _BOSS.checkBiKhongChe()
                                                || _BOSS.isdie || _BOSS.countCharge >= 10) {
                                            _BOSS.countCharge = 0;
                                            _BOSS.isTTNL = false;
                        ...
                        if (_charTarget != null) {
                            if (_BOSS._typeBoss < (byte) 7) {
                                ...
                                String[] text = {"Ngươi là ai?","Chịu chết đi","Muốn xâm chiếm hành tinh này sao?"};
                                chat(_BOSS, text[Util.nextInt(0, text.length)]);
                            } else if (_BOSS._typeBoss >= (byte) 52 && _BOSS._typeBoss <= (byte) 55) {
                                String[] text = {"Chiếm lấy hành tinh này?","Hành tinh này bán được bao nhiêu đây, haha","Muốn cản ta sao?"};
                                chat(_BOSS, text[Util.nextInt(0, text.length)]);
                            }
                        }
                    }
...
//        short minSpace = 150;
    ...
    public ItemMap dropItemGOD(Player p, short xDROP, short yDROP, byte type) {
        ItemMap itemROI = null;
        if(Util.nextInt(0,2) == 1) {
            int perDrop = Util.nextInt(0, 100);

            if(perDrop >= 60) {
                int idNRRot = 15;
//                boolean isSuperBoss = (type == (byte)4 || type == (byte)6 || type == (byte)30 || type == (byte)48) ? true : false;
                if (perDrop < 90 && perDrop > 60) { //1s
                    idNRRot = 14;
                } else if (perDrop >= 90 && perDrop < 95) { //2s
                    idNRRot = 15;
                } else if (perDrop >= 95 && perDrop < 98) { //thoi vang
                    idNRRot = 457;
//                } else if (perDrop >= 98 && perDrop < 99) { //do than linh
                } else if (perDrop == 98) { //do than linh
                    int idItemGod[] = {555, 556, 561, 562, 563, 557, 558, 561, 564, 565, 559, 560, 561, 566, 567};
                    idNRRot = idItemGod[Util.nextInt(0, idItemGod.length)];
                }
                //            else { //do huy diet
                //                int idItemGod[] = {650,651,657,658,656, 652,653,659,660,656, 654,655,661,662,656};
                //                idNRRot =  idItemGod[Util.nextInt(0, idItemGod.length)];
                //            }

                if (idNRRot >= 555 && idNRRot <= 567) {
                    Item itemGOD = ItemSell.getItemNotSell(idNRRot);
                    itemROI = new ItemMap();
                    itemROI.playerId = p.id;
                    itemROI.x = xDROP;
                    itemROI.y = yDROP;
                ...
                //            else if(idNRRot >= 650 && idNRRot <= 662)
                //            {
                //                Item itemGOD = ItemSell.getItemNotSell(idNRRot);
                //                itemROI = new ItemMap();
                //                itemROI.playerId = p.id;
                ...
                //                //BUILD NEW ITEM + CHI SO CHO DO THAN LINH
                //                assert itemGOD != null;
                //                itemROI.item = new Item(itemGOD);
                //                return itemROI;
                //            }
                else {
                    Item itemMap = ItemSell.getItemNotSell(idNRRot);
                    //            if(idNRRot == 457) {
                    //                itemMap.itemPay = true;
                    //            }
                    itemROI = new ItemMap();
                    itemROI.playerId = p.id;
                    itemROI.x = xDROP;
                    ...
                    itemROI.item = new Item(itemMap);
                    if (idNRRot == 457) {
                        itemROI.item.itemPay = true;
                    }
    ...
    public ItemMap dropItemGodCereal(Player p, short xDROP, short yDROP) {
        ItemMap itemROI = null;
        int idNRRot = Constant.ITEM_GOD[Util.nextInt(0, Constant.ITEM_GOD.length)];
        ...
        if (idNRRot >= 555 && idNRRot <= 567) {
            ...
            //ti le ra do sao pha le option 102
            idNRRot = Util.nextInt(0, 100);
            if(idNRRot <= 16) {
                idNRRot = 0;
            } else if(idNRRot > 16 && idNRRot <= 26) {
                idNRRot = 1;
            } else if(idNRRot > 26 && idNRRot <= 46) {
                idNRRot = 2;
            } else if(idNRRot > 46 && idNRRot <= 66) {
                idNRRot = 3;
            } else if(idNRRot > 66 && idNRRot <= 81) {
                idNRRot = 4;
            } else if(idNRRot > 81 && idNRRot <= 96) {
                idNRRot = 5;
            } else if(idNRRot > 96 && idNRRot <= 98) {
                idNRRot = 6;
            } else if(idNRRot > 98) {
                idNRRot = 7;
            }
            _ITEMGOD.itemOptions.add(new ItemOption(21, 17));
            if(idNRRot > 0 && idNRRot < 8) {
                _ITEMGOD.itemOptions.add(new ItemOption(107, idNRRot));
            }
            itemROI.item = _ITEMGOD;
            return itemROI;
        }
...
//            if(itemMap.itemMapID == item.itemMapID) {
    ...
    public ItemMap createNewItemMap(int idItem, int idPlayer, short x, short y) {
        ...
        if(this.itemsMap.size() > 0) {
            itemM.itemMapID = (this.itemsMap.get(this.itemsMap.size() - 1).itemMapID + 1);
            if(itemM.itemMapID >= 50000) {
                itemM.itemMapID = 0;
            }
        } else {
...
//        }
    ...
    public void resetBagClan(@NotNull Player player) {
        byte imgClan = -1;
        if(player.clan != null) {
...
//                Util.log("count: " + count);
    ...
    //INIT BIG BOSS AUTO
    public void initBigBoss(@NotNull Mob bigBoss) {
...
//            }
    ...
    //INIT MOB CHIM
    public void initChimNamek(@NotNull Player p, int idTemp) {
    ...
    public void timerDeleteBoss(@NotNull Boss _bTarget) {
