...
public class ClanService {
    ...
    public void clanInfo(Session session, @NotNull Player pl) {
        Message msg;
        Clan clan = pl.clan;
        try {
            msg = new Message(-53);
...
//        name = Util.validateClan(name);
    ...
    public void getBagBangNew(Session session, byte idBag) {
        ...
        try {
            ...
            } else if(idBag == (byte)83) {//CANH DAO REAL
                m.writer().writeByte((byte)2);
                m.writer().writeShort((short)11023);
                m.writer().writeShort((short)11024);
            } else if(idBag == (byte)84) {//CANH MAI REAL
                m.writer().writeByte((byte)2);
                m.writer().writeShort((short)11026);
                m.writer().writeShort((short)11027);
            }
...
//            m.writer().writeByte((byte)10);
    ...
    public Member createNewMember(@NotNull Player player, byte roleMember) {
        //ADD LEADER VAO NEW CLAN
        Member member = new Member();
        member.id = player.id;
        member.name = player.name;
    ...
    public void sendMessageToClan(@NotNull Clan clan) {
...
//            System.out.println("MESSAGE CLAN " + i + " ID: " + clan.messages.get(i).id + ", TYPE: " + clan.messages.get(i).type + "PLAYER: " + clan.messages.get(i).playerName + "TEXT: " + clan.messages.get(i).text);
    ...
    public void messageXinVaoBANG(@NotNull Clan clan, ClanMessage clanMess) {
        for(Member _member: clan.members) {
            Player _playerSend = PlayerManger.gI().getPlayerByUserID(_member.id);
            if(_playerSend != null) {
    ...
    public void checkHasMessageClan(@NotNull Clan clan, int playerid) {
    ...
    public void changeRolePhoBang(int idPlayer, @NotNull Player player, byte roleNew) {
...
//        ClanDAO.removeClanMember(idPlayer);
    ...
    //ROI KHOI BANG HOI
    public void leaveThanhVien(@NotNull Player player) {
        //GET CLAN SE ROI
...
//        ClanDAO.removeClanMember(idPlayer);
    ...
    //GIAI TAN BANG HOI
    public void distoryClan(@NotNull Player p) {
...
//            clanDelete.flagDestroy = true;
    ...
    public void setupNRSDClan(@NotNull Player p, byte id) {
    ...
    public void updateDataClanToDBv2(Connection conn) {
        ClanDAO.deleteAllClan(conn);

        for (Clan clan : ClanManager.gI().getClans()) {
            ClanDAO.createNewClan(conn, clan);

            for(Member mem: clan.members) {
                if(mem.id != clan.leaderID) {
                    ClanDAO.addNewMember(conn, clan.id, mem);
                }
            }
        }
    }
