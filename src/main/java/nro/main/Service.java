...
//            PlayerManger.gI().getPlayers().add(player.detu);
    ...
    public void LoadCaiTrang(@NotNull Player p , int type , int head , int body , int leg){
//        System.out.println("ismonkey ao that day: " + p.isMonkey);
    ...
    public void LoadBodyPlayerChange(@NotNull Player _player , int type , int head , int body , int leg){
        Message m = null;
        try {
            m = new Message(-90);
            m.writer().writeByte(type);
    ...
    public void LoadBodyDetuChange(@NotNull ArrayList<Player> listPlayer, int type , int idpet, int head , int body , int leg){
        Message m = null;
        try {
            m = new Message(-90);
            m.writer().writeByte(type);
    ...
    public void sendTB(@NotNull Session p, int avatar, String s , int typechat) {
        Message m = null;
        try {
            m = new Message(-70);
    ...
    //SEND EFFECT
    public void sendEffectServer(byte loop, byte layer, byte ideff, short x, short y, short loopcount, @NotNull Player p) {
        Message m = null;
        try {
            m = new Message(113);
...
//            msg.writer().writeInt(5714013);
    ...
    public void sendMessage(Session session, int cmd, String filename) {
        ...
        try {
            msg = new Message(cmd);
            msg.writer().write(Objects.requireNonNull(FileIO.readFile("data/msg/" + filename)));
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception ignored) {
        }
    ...
    public void sendNewMount(Session session, String filename) {
        ...
        try {
            msg = new Message(66);
            msg.writer().write(Objects.requireNonNull(FileIO.readFile("data/mount/x" + session.zoomLevel + "/" + filename)));
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception ignored) {
        }
    ...
    public void updateData(Session session) {
        ...
        try {
            msg = new Message(-87);
            msg.writer().write(Objects.requireNonNull(FileIO.readFile("data/1632811838531_-87_r")));
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception ignored) {
        }
...
//            msg.writer().write(FileIO.readFile("data/NRmap_v25"));
            msg.writer().write(Objects.requireNonNull(FileIO.readFile("data/1632811838538_-28_6_r")));
...
//            msg.writer().write(FileIO.readFile("data/NRskill_v5"));
            msg.writer().write(Objects.requireNonNull(FileIO.readFile("data/1632811838545_-28_7_r")));
//            session.doSendMessage(msg);
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception ignored) {
...
//        System.out.println("update item");
    ...
    public void tileSet(Session session, int id) {
        ...
        try {
            //msg = messageNotMap((byte) 6);
            msg = new Message(-82);
            msg.writer().write(Objects.requireNonNull(FileIO.readFile("data/map/tile_set/" + id)));
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception ignored) {
        }
    ...
    public void tileSetReal(Session session) {
        ...
        try {
            //msg = messageNotMap((byte) 6);
            msg = new Message(-82);
            msg.writer().write(Objects.requireNonNull(FileIO.readFile("data/map/tile_set_real/20220812195141001_-82_r")));
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception ignored) {
        }
    ...
    public void mapTemp(Session session, int id) {
        ...
        try {
            ...
            msg = new Message(-28);
            if(id == 166 || id == 167) {
                msg.writer().writeByte((byte)10);
            }
            msg.writer().write(Objects.requireNonNull(FileIO.readFile("data/map/temp/" + id)));
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception ignored) {
        }
    ...
    public void updateSloganClan(@NotNull Session session, String text) {
...
//            msg.writer().writeInt(pl.vang);
            ...
            //TODO HEAD NEW
            ByteArrayInputStream is = new ByteArrayInputStream(Objects.requireNonNull(FileIO.readFile("res/cache/v225/head")));
            DataInputStream dis = new DataInputStream(is);
            short lengthHead = dis.readShort();
            msg.writer().writeShort((short)302);
            for(short i = 0; i < lengthHead; i++) {
                msg.writer().writeShort(dis.readShort());
                msg.writer().writeShort(dis.readShort());
            }
            msg.writer().writeShort((short)1197);
            msg.writer().writeShort((short)11061);
            msg.writer().writeShort((short)1179);
            msg.writer().writeShort((short)11074);

//            ByteArrayInputStream is = new ByteArrayInputStream(Objects.requireNonNull(FileIO.readFile("res/cache/vhalloween/head")));
//            DataInputStream dis = new DataInputStream(is);
//            short lengthHead = dis.readShort();
...
//            m.writer().writeShort(p.Leg());
    ...
    private @NotNull Message messageNotLogin(byte command) throws IOException {
        Message ms = new Message(-29);
        ms.writer().writeByte(command);
        return ms;
    }

    private @NotNull Message messageNotMap(byte command) throws IOException {
        Message ms = new Message(-28);
        ms.writer().writeByte(command);
        return ms;
    }

    private @NotNull Message messageSubCommand(byte command) throws IOException {
...
//                m.writer().writeShort(2330);
    ...
    //send pet hoi sinh
    public void petLiveFromDead(@NotNull Player _player) {
    ...
    //send pet hoi sinh
    public void SendPetLiveFromDead(@NotNull Player _player) {
        _player.zone.chat(_player.detu, "Sư phụ ơi con đây rồi!");
        Message m = null;
        try {
            _player.detu.hp = _player.detu.getHpFull();
    ...
    //PPET COMEBACK
    public void petComebackLogin(@NotNull Player p) {
...
//                    SendPetLiveFromDead(p); //dang tan cong nen hoi sinh tai cho
    ...
    public void openUITransport(@NotNull Player player) {
    ...
    public void openUISaoDen(@NotNull Player p) {
        p.moveToSaoDen = true;
        Message m = null;
        int i = 0;
        try {
    ...
    //NANG CAP ITEM
    public void EpStarItem(@NotNull Player _player, byte _index, byte _index2) {
        ...
        if((_item.template.type == 0 || _item.template.type == 1 || _item.template.type == 2 || _item.template.type == 3 || _item.template.type == 4
                || _item.template.type == 5 || _item.template.type == 32) &&
            ...
            _player._indexEpStar = _index2;
               ...
               || _item.template.type == 5 || _item2.template.type == 32 ) &&
...
//            Util.log("STRING INFO ITEM: " + _info);
    ...
    public void sendEpStarItem(@NotNull Player _player) {
        ...
        if(_itemUp.template.type != 0 && _itemUp.template.type != 1 && _itemUp.template.type != 2 && _itemUp.template.type != 3 && _itemUp.template.type != 4
                && _itemUp.template.type != 5  && _itemUp.template.type != 32 ) {
            return;
        }
...
//                                _player.ItemBag[_player._indexUpStar].itemOptions.get(i).param += 1;
                                _player.ItemBag[_player._indexUpStar].itemOptions.get(i).param = Math.min((_player.ItemBag[_player._indexUpStar].itemOptions.get(i).param + 1), 8);
                                chkAddUseDone = true;
                                break;
                            }
                        }
                    ...
                    for(int i = 0; i < _player.ItemBag[_player._indexUpStar].itemOptions.size(); i++) {
                        if(_player.ItemBag[_player._indexUpStar].getParamItemByID(102) == 0) {
                            ...
                            break;
                        } else if(_player.ItemBag[_player._indexUpStar].itemOptions.get(i).id == 102) { //update so sao phale da ep
//                            _player.ItemBag[_player._indexUpStar].itemOptions.get(i).param += 1;
                            _player.ItemBag[_player._indexUpStar].itemOptions.get(i).param = Math.min((_player.ItemBag[_player._indexUpStar].itemOptions.get(i).param + 1), 8);
                            chkAddUseDone = true;
                            for(int j = 0; j < _player.ItemBag[_player._indexUpStar].itemOptions.size(); j++) {
    ...
    public void upgradeStarItem(@NotNull Player _player, byte _index) {
        Item _item = _player.ItemBag[_index];
        if(_item == null) {
            return;
        }
    ...
    public void sendUpStarItem(@NotNull Player _player) {
        ...
        if(_itemUp.template.type != 0 && _itemUp.template.type != 1 && _itemUp.template.type != 2 && _itemUp.template.type != 3 && _itemUp.template.type != 4
            ...
            if((System.currentTimeMillis() - _player._timeDapDo) >= 1000 && _player._checkDapDo) {
                _player._timeDapDo = System.currentTimeMillis();
                _player._checkDapDo = false;
                if(_player.vang >= _itemUp.getGoldUpStar(_itemUp._starItem) * 1000000L && _player.ngoc >= _itemUp._starItem + 1 && _itemUp.getParamItemByID(107) < 8) {
                    _player.vang -= (_itemUp.getGoldUpStar(_itemUp._starItem)*1000000L);
                    _player.ngoc -= (_itemUp._starItem + 1);
                    int rdUp = Util.nextInt(0, 300);
                    if(_itemUp._starItem == 5) {
                        rdUp = Util.nextInt(0, 600);
                    } else if(_itemUp._starItem == 6) {
                        rdUp = Util.nextInt(0, 800);
                    } else if(_itemUp._starItem == 7) {
                        rdUp = Util.nextInt(0, 1200);
                    }
    ...
    public void upgradeLevelItem(@NotNull Player _player, byte _index, byte _index2, byte _index3) {
        ...
        if(lvItem < 8 && !Objects.equals(_info, "")) {
...
//            Util.log("INFOOOOOOOOOOOOOOOOO: " + _info);
    ...
    public void sendUpLevelItem(@NotNull Player _player) {
        Item _itemUp = _player._itemUpStar;
        ...
        if((System.currentTimeMillis() - _player._timeDapDo) >= 1000 && _player._checkDapDo) {
            _player._timeDapDo = System.currentTimeMillis();
            _player._checkDapDo = false;
            if(_itemUp != null && _stone != null && (_player.vang >= (_itemUp.getGoldUpItem(_itemUp._levelItem)* 1000L) ) && (_player.ItemBag[_player._indexEpStar].quantity >= _itemUp.getCountStoneUpItem(_itemUp._levelItem) ) ) {
                _player.vang -= (_itemUp.getGoldUpItem(_itemUp._levelItem)* 1000L);
                _player.ItemBag[_player._indexEpStar].quantity -= _itemUp.getCountStoneUpItem(_itemUp._levelItem);
                if(_player.ItemBag[_player._indexEpStar].quantity == 0) {
                    _player.ItemBag[_player._indexEpStar] = null;
                }
                int rdUp = Util.nextInt(0, 150);
    ...
    public void resetItemDapDo(@NotNull Player p) {
        p._itemUpStar = null;
        p._indexUpStar = -1;
    ...
    public void upgradePorata(@NotNull Player _player, byte _index, byte _index2) {
        Item _item = _player.ItemBag[_index]; //get item 1
        Item _item2 = _player.ItemBag[_index2]; //get item 2
        String _info = "";
        if(_item == null || _item2 == null) {
        ...
        if((_item.template.id == 933 && _item2.template.id == 454) || (_item2.template.id == 933 && _item.template.id == 454)) {
            ...
            if(_item.template.id == 933 && _item.getParamItemByID(31) >= 9999) {
                _player._itemUpStar = _item2;
                _player._itemUseEpStar = _item;
                _player._indexUpStar = _index2;
                _player._indexEpStar = _index;
    ...
    public void sendUpPorata(@NotNull Player _player) {
        Item _itemUp = _player._itemUpStar;
        Item _stone = _player._itemUseEpStar;
        Message m = null;
        ...
        if((System.currentTimeMillis() - _player._timeDapDo) >= 1000 && _player._checkDapDo) {
            _player._timeDapDo = System.currentTimeMillis();
            _player._checkDapDo = false;
            if((_player.vang >= 500000000) && (_player.ngoc >= 5000) && (_player.ItemBag[_player._indexEpStar].getParamItemByID(31) >= 9999) ) {
                ...
                if(rdUp <= 35) {
                    ...
                    Item _PORATA2 = new Item(ItemSell.getItemNotSell(921));
    ...
    //MO CHI SO PORATA 2
    public void openOptionPorata(@NotNull Player _player, byte _index, byte _index2, byte _index3) {
        Item _item = _player.ItemBag[_index]; //get item 1
        Item _item2 = _player.ItemBag[_index2]; //get item 2
    ...
    public void sendOpenOptionPorata(@NotNull Player _player) {
        Item _itemUp = _player._itemUpStar;
        ...
        if((System.currentTimeMillis() - _player._timeDapDo) >= 1000 && _player._checkDapDo) {
            _player._timeDapDo = System.currentTimeMillis();
            _player._checkDapDo = false;
            if((_player.ngoc >= 2500) && (_player.ItemBag[_player._indexEpStar].quantity >= 99) && (_player.ItemBag[_player._indexEpStar2].quantity >= 1) ) {
                _player.ngoc -= 2500;
                _player.ItemBag[_player._indexEpStar] = null;
                ...
                if(rdUp <= 35) {
                    ...
                    if(_player.ItemBag[_player._indexUpStar].template.id == 921) {
                        ...
                        int paramOption = Util.nextInt(0, 100) < 80 ? Util.nextInt(1, 8) : Util.nextInt(8, 16);
                        if(rdUp == 4 || rdUp == 5) {
                            paramOption = Util.nextInt(0, 100) < 80 ? Util.nextInt(2, 8) : Util.nextInt(8, 11);
                        }
                        if(rdUp == 0) {
                            _player.ItemBag[_player._indexUpStar].itemOptions.add(new ItemOption(50, paramOption));
                        } else if(rdUp == 1) {
                            _player.ItemBag[_player._indexUpStar].itemOptions.add(new ItemOption(77, paramOption));
                        } else if(rdUp == 2) {
                            _player.ItemBag[_player._indexUpStar].itemOptions.add(new ItemOption(103, paramOption));
                        } else if(rdUp == 3) {
                            _player.ItemBag[_player._indexUpStar].itemOptions.add(new ItemOption(108, paramOption));
                        } else if(rdUp == 4) {
                            _player.ItemBag[_player._indexUpStar].itemOptions.add(new ItemOption(94, paramOption));
                        } else if(rdUp == 5) {
                            _player.ItemBag[_player._indexUpStar].itemOptions.add(new ItemOption(14, paramOption));
                        } else if(rdUp == 6) {
                            _player.ItemBag[_player._indexUpStar].itemOptions.add(new ItemOption(80, paramOption));
                        } else if(rdUp == 7) {
                            _player.ItemBag[_player._indexUpStar].itemOptions.add(new ItemOption(81, paramOption));
                        } else if(rdUp == 8) {
                            _player.ItemBag[_player._indexUpStar].itemOptions.add(new ItemOption(175, paramOption));
                        }
                    }
                    sendUpStarSuccess(_player);
                } else {
    ...
    //NANG CAP ITEM HEART
    public void upgradeItemHeart(@NotNull Player _player, byte _index, byte _index2) {
        Item _item = _player.ItemBag[_index]; //get item 1
        Item _item2 = _player.ItemBag[_index2]; //get item 2
        String _info = "";
        if(_item == null || _item2 == null) {
    ...
    public void sendUpItemHeart(@NotNull Player _player) {
        Item _itemUp = _player._itemUpStar;
        Item _stone = _player._itemUseEpStar;
        Message m = null;
    ...
    //EP MANH DA VUN
    public void EpStoneBreak(@NotNull Player _player, byte _index1, byte _index2) {
    ...
    public void sendEpDaVun(@NotNull Player _player) {
    ...
    public void EpDragonBall(@NotNull Player _player, byte _index) {
...
//        if(_item.template.id >= 15 && _item.template.id <=20) { //2s den 7s
    ...
    public void sendEpNgocRong(@NotNull Player _player) {
...
//            m.writer().writeInt(5);
    ...
    public void coolDownAllSkill(Player p) {
        ...
        try {
            ...
            for(Skill skill: p.skill) {
                time = Math.max((int) (skill.coolDown - (System.currentTimeMillis() - skill.lastTimeUseThisSkill)), 0);
    ...
    public boolean checkCanAttackChar(Player player, @NotNull Player _pTarget) {
    ...
    public boolean checkCanAttackBoss(@NotNull Boss boss) {
    ...
    public boolean checkCanAttackDeTu(Player player, @NotNull Detu _pTarget) {
    ...
    public void initKuKu() {
        ...
        int IDZONE = Util.nextInt(0, Server.maps[idMap].area.length);
        if(idMap == 69) {
            xKu = (short)808;
            yKu = (short)384;
        } else if(idMap == 70) {
        ...
        Server.maps[idMap].area[IDZONE].bossMap.add(_rKuku);
        Server.maps[idMap].area[IDZONE].loadBossNoPet(_rKuku);
        Server.gI().mapKUKU = idMap;
        Server.gI().khuKUKU = IDZONE;
        sendThongBaoServer("BOSS " + _rKuku.name + " vừa xuất hiện tại " + Server.maps[idMap].template.name);
...
//        Util.log("INIT _rRambo XONG MAP " + Server.gI().maps[idMap].template.name + ", " + IDZONE);
    ...
    public void initTDST()
    {
        ...
        int IDZONE = Util.nextInt(1, Server.gI().maps[idMap].area.length);
...
//        sendThongBaoServer("BOSS " + _Android19.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name);
        ...
        _Android20.typePk = (byte)1;
        Server.gI().maps[idMap].area[IDZONE].bossMap.add(_Android20);
        Server.gI().maps[idMap].area[IDZONE].loadInfoBoss(_Android20);
//        Server.gI().maps[idMap].area[IDZONE].loadBossNoPet(_Android20);

        Server.gI().map1920 = idMap;
        Server.gI().khu1920 = IDZONE;
//        sendThongBaoServer("BOSS " + _Android20.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name);
    ...
    {
        int IDZONE = Util.nextInt(1, Server.gI().maps[101].area.length);
...
//        sendThongBaoServer("BOSS " + _Android15.name + " vừa xuất hiện tại " + Server.gI().maps[101].template.name);
        ...
        _Android14.typePk = (byte)1;
        _Android13.typePk = (byte)1;
        Server.gI().maps[101].area[IDZONE].bossMap.add(_Android14);
        Server.gI().maps[101].area[IDZONE].loadInfoBoss(_Android14);
...
//        sendThongBaoServer("BOSS " + _Android14.name + " vừa xuất hiện tại " + Server.gI().maps[101].template.name);
        ...
        Server.gI().maps[101].area[IDZONE].loadInfoBoss(_Android13);

        Server.gI().khu151413 = IDZONE;
...
//        Util.log("INIT initBillWhis XONG MAP " + Server.gI().maps[5].template.name + ", " + IDZONE);
    ...
    public void initGranolal() {
        int idMap = 162;
        short x = (short)730;
        short y = (short)144;
        int zoneId = Util.nextInt(0, Server.gI().maps[idMap].area.length);

        Boss granolal = new Boss(700, (byte)51, x, y);
        Server.gI().maps[idMap].area[zoneId].bossMap.add(granolal);
        Server.gI().maps[idMap].area[zoneId].loadBossNoPet(granolal);
        Server.gI().mapRAMBO = idMap;
        Server.gI().khuRAMBO = zoneId;
        sendThongBaoServer("BOSS " + granolal.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name);
        Util.log("INIT granolal XONG MAP " + Server.gI().maps[idMap].template.name + ", " + zoneId);
    }

    public void initHeeter()
    {
        int idMap = 163;
        short xSo4 = (short)636;
        short ySo4 = (short)312;
        short xSo3 = (short)696;
        short xSo2 = (short)756;
        short xSo1 = (short)816;

        int IDZONE = Util.nextInt(0, Server.gI().maps[idMap].area.length);
        Boss _So4 = new Boss(701, (byte)53, xSo4, ySo4);
        Boss _So3 = new Boss(702, (byte)54, xSo3, ySo4);
        Boss _So2 = new Boss(703, (byte)55, xSo2, ySo4);
        Boss _So1 = new Boss(704, (byte)52, xSo1, ySo4);
        ...
        Util.log("INIT Heeter XONG MAP " + Server.gI().maps[idMap].template.name + ", " + IDZONE);
    }
            ...
            public void run()
            {
                for(int i = 84; i < 91; i++) {
                    for(int j = 0; j < Server.gI().maps[i].area.length; j++) {
                        ...
                        if(Server.gI().maps[i].area[j].itemsMap.size() > 0) {
                            item.itemMapID = (Server.gI().maps[i].area[j].itemsMap.get(Server.gI().maps[i].area[j].itemsMap.size() - 1).itemMapID + 1);
...
//                    idZone = 0;
                    ...
                    if(Server.gI().maps[listMap.get(index)].area[idZone].itemsMap.size() > 0) {
                        item.itemMapID = (Server.gI().maps[listMap.get(index)].area[idZone].itemsMap.get(Server.gI().maps[listMap.get(index)].area[idZone].itemsMap.size() - 1).itemMapID + 1);
                        if(item.itemMapID >= 50000) {
                            item.itemMapID = 0;
                        }
                    } else {
...
//                _p.itemLung = null;
    ...
    public void teleportToNrNamec(@NotNull Player p) {
    ...
    public void dropDragonBall(@NotNull Player p) {
...
//        calendar.set(Calendar.HOUR_OF_DAY, 10); //SERVER 2
    ...
    //DOANH TRAI
    public void initMapDoanhTrai(@NotNull Clan clan, int idMap, byte index) {
...
//            ClanDAO.updateTimeOpenDT(clan.id);
    ...
    public void destroyGiaoDich(@NotNull Player player) {
...
//            Service.gI().buyDone(player._friendGiaoDich);
    ...
    public String writePartBodyLeg(@NotNull Player player) {
        if(player.isMonkey) {
            return "193,194";
...
//            m.writer().writeShort(player.Leg());
    ...
    public String writePartBodyLegDetu(@NotNull Detu player) {
        if(player.isMonkey) {
            return "193,194";
        } else if((player.ItemBody[5] != null && player.ItemBody[5].template.level != 0 && player.ItemBody[5].template.part == (short)(-1) && (player.ItemBody[5].template.id != 601 && player.ItemBody[5].template.id != 602 && player.ItemBody[5].template.id != 603 
&& player.ItemBody[5].template.id != 639 && player.ItemBody[5].template.id != 640 && player.ItemBody[5].template.id != 641)) || player.NhapThe == 1 || player.isSoSinh) {
    ...
    public void loadCaiTrangTemp(@NotNull Player _bTarget) {
    ...
    //LEAVE ALL
    public void leaveOutMap(@NotNull Player p) {
        if(p.petfucus == 1) {
            p.zone.leaveDetu(p, p.detu);
        }
        if(p.pet2Follow == 1 && p.pet != null) {
    ...
    //*************************************INIT KHI GA****************************
    public void initKhiGa(@NotNull Clan clan, int level) {
//        MapTemplate doanhTraiTemp = map.template.getMapTemplateByIDMap(53);
...
//        Util.log("AJCJKAHCKA :" + level);
    ...
    public void initLychee(@NotNull Player p) {
    ...
    //***********************************GIAP LUYEN TAP***********************
    public void upTimeGLT(@NotNull Player p) {
...
//            Util.log("CUPTIME GLT: " + p.ItemBody[6].cUpTimeGLT + ", TIME: " + p.ItemBody[6].timeGLT);
    ...
    public void timeGiapLuyenTap(@NotNull Player p, byte type) {
        ...
        TimerTask giapLT = new TimerTask() {
            public void run()
            {
                ...
                int timeDown = Math.max((int) (p.giapLuyenTap.timeGLT / 60000), 0);
    ...
    public void teleportByTauNgam(@NotNull Player p, int idMap, long time) {
        Timer timerTauNgam = new Timer();
        TimerTask tauNgam = new TimerTask() {
            public void run()
            {
    ...
    public boolean checkTaskGoMap(@NotNull Player p, int mapid) {
    ...
    //USE ITEM LUC LOGIN
    public void sendItemBuff(@NotNull Player p) {
        if(p.timerCSKB != null) {
...
//            showIconBuff(p, (p.idTAUse + 5661), p.secondTA);
    ...
    public void itemBuffLogin(@NotNull Player p, int id, long time) {
    ...
    public void initPet2(@NotNull Player p, int id) {
    ...
    public long getTnsmAdd(@NotNull Player p, long tnsm) {
        long upSmTn = (long) (tnsm *3);
...
//                        }
        ...
        if (p.power >= 180000000000L) {
            upSmTn = upSmTn < 25000 ? 25000 : upSmTn;
        }
        upSmTn = upSmTn > 1 ? upSmTn : 1;
    ...
    public void updateTaskPower(@NotNull Player p) {
    ...
    public void updateTaskFarmMob(@NotNull Player p, @NotNull Mob mob) {
...
//                            }
    ...
    public void updateTaskKillBoss(@NotNull Player p, Boss _bTarget) {
        ...
        if(p.taskId == (short)31 && p.crrTask.index == (byte)0 && _bTarget._typeBoss == (byte)6) {
            int perNhan = Util.nextInt(0, 1);
            if(perNhan == 0) {
                Item itemNhan =  ItemSell.getItemNotSell(992);
                assert itemNhan != null;
        ...
        if(_bTarget._typeBoss >= (byte)36 && _bTarget._typeBoss <= (byte)39) {
            p.pointMabu = (byte)10;
            setPowerPoint(p, "TL", (short)10, (short)10, (short)10);
        }
        //END CHECK SP MABU
    }

    public int getDameSetActive(@NotNull Player p,@NotNull Skill skillPlayerUse, int damage) {
        ...
        if (p.getSetKichHoatFull() == (byte) 3 && skillPlayerUse.template.id == (byte) 1) {
            ...
            damage = (int)(damage*Util.getPercentDouble(200 + p.getDameUpItemActive()));
        } else if(p.getSetKichHoatFull() == (byte) 10 && skillPlayerUse.template.id == (byte)17) { //SET KICH HOAT LIEN HOAN
            damage = (int)(damage*Util.getPercentDouble(200 + p.getDameUpItemActive()));
        }
